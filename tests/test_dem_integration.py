...
class DemIntegrationTests(unittest.TestCase):
    ...
    def test_terrain_tab_enables_ga_downloader_for_a_vector_extent(self):
        ...
        with patch(
            ...
            self.assertEqual(dialog.comboBox_dem_source.minimumHeight(), 32)
            self.assertEqual(dialog.comboBox_dem_source.maximumHeight(), 32)
            self.assertEqual(dialog.pushButton_dem_extent_override.height(), 32)
            self.assertEqual(dialog.doubleSpinBox_dem_contour_interval.height(), 32)
            self.assertEqual(dialog.pushButton_DownloadDem.height(), 36)
            self.assertEqual(dialog.pushButton_CreateDemContours.height(), 36)
            self.assertEqual(dialog.comboOutputFormat.minimumHeight(), 32)
            self.assertEqual(dialog.comboOutputFormat.maximumHeight(), 32)
            self.assertEqual(dialog.pushButton_add_CNS.minimumHeight(), 32)
            self.assertEqual(dialog.pushButton_add_CNS.maximumHeight(), 32)
            self.assertEqual(dialog.pushButton_DownloadOsmAeroway.minimumHeight(), 36)
            self.assertEqual(dialog.pushButton_DownloadOsmAeroway.maximumHeight(), 36)
            self.assertEqual(dialog.pushButton_Generate.minimumHeight(), 38)
            self.assertEqual(dialog.pushButton_Generate.maximumHeight(), 38)
