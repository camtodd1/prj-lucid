...
class RuntimeDashboardTests(unittest.TestCase):
    def test_scenario_dimensions_are_available_to_every_dashboard_control(self):
        ...
        self.assertIn("candidate.trendSetup === run.trendSetup", html)
        self.assertNotIn("candidate.exactSetup === run.exactSetup", html)
        self.assertIn("'Exact setup'", html)
        self.assertIn("'Setup varied'", html)
        self.assertIn("'Setup details unavailable'", html)
        self.assertIn("'No prior matching trend'", html)
