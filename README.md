...
- OpenStreetMap features carrying the `aeroway` key within 5 km of the entered
  ...
  endpoint. Downloads run as background QGIS tasks while a modal busy dialog
  shows the active server attempt and layer-preparation status.
...
- UK consultation circles and DfT Public Safety Zones are indicative defaults;
  officially lodged aerodrome/operator maps control.
