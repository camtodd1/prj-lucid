...
class SafeguardingBuilder:
    ...
    def __init__(self, iface):
        ...
        # ### NEW: Instance variables for output options ###
    ...
    def run(self):
        ...
        if self.dlg is None:
          ...
          try:
            ...
            QgsMessageLog.logMessage(f"Error creating dialog: {e}\n{traceback.format_exc()}", PLUGIN_TAG, level=Qgis.Critical)
            QMessageBox.critical(parent_window, self.tr("Dialog Error"), self.tr("Could not create dialog. Check logs."))
            return
          ...
          if generate_button:
            ...
            QgsMessageLog.logMessage("CRITICAL UI ERROR: 'Generate Safeguarding Surfaces' button (expected objectName 'pushButton_Generate') not found in dialog.", PLUGIN_TAG, level=Qgis.Critical)
            QMessageBox.critical(self.dlg, self.tr("UI Error"), self.tr("Main 'Generate' button is missing from the dialog. Plugin cannot function."))
    ...
    def _sanitize_filename(self, name: str, replace_char: str = '_') -> str:
      ...
      if not name: name = "unnamed_layer" # Fallback if sanitization results in empty string
      return name
    ...
    def _create_and_add_layer(self, geometry_type_str: str, internal_name_base: str,
      ...
      if not target_crs or not target_crs.isValid():
        ...
        return None
      if not layer_group:
        QgsMessageLog.logMessage(f"Cannot add layer '{display_name}': Invalid layer group provided.", plugin_tag, level=Qgis.Critical)
        return None
      ...
      if self.output_mode == 'memory':
        ...
        if not layer or not layer.isValid():
          QgsMessageLog.logMessage(f"Failed create valid memory layer object for '{display_name}'.", plugin_tag, level=Qgis.Warning)
          return None
        
        provider = layer.dataProvider()
        if not provider or not provider.addAttributes(fields):
          QgsMessageLog.logMessage(f"Failed setup provider/attributes for memory layer '{display_name}'. Provider exists: {provider is not None}", plugin_tag, level=Qgis.Warning)
          return None
        layer.updateFields()
          ...
          if add_ok:
            if layer.commitChanges():
              ...
              if node:
                ...
                QgsMessageLog.logMessage(f"Warning: Memory layer '{display_name}' added to project but failed add node to group '{layer_group.name()}'.", plugin_tag, level=Qgis.Warning)
                if project.mapLayer(layer.id()): project.removeMapLayer(layer.id())
            else:
              QgsMessageLog.logMessage(f"Failed commit changes for memory layer '{display_name}'. Error: {layer.commitErrors()}", plugin_tag, level=Qgis.Warning)
              layer.rollBack()
          else:
            QgsMessageLog.logMessage(f"Failed addFeatures for memory layer '{display_name}'.", plugin_tag, level=Qgis.Warning)
            layer.rollBack()
        else:
          QgsMessageLog.logMessage(f"Failed startEditing for memory layer '{display_name}'.", plugin_tag, level=Qgis.Warning)
          
      # --- FILE OUTPUT MODE ---
      elif self.output_mode == 'file':
        if not all([self.output_path, self.output_format_driver, self.output_format_extension]):
          QgsMessageLog.logMessage(f"Skipping file save for '{display_name}': Output path/driver/extension missing.", plugin_tag, level=Qgis.Critical)
          return None
        ...
        if not temp_provider or not temp_provider.addAttributes(fields):
          QgsMessageLog.logMessage(f"Failed setup provider/attributes for temp layer '{display_name}'.", plugin_tag, level=Qgis.Warning)
          return None
        temp_layer.updateFields()
        
        add_temp_ok, _ = temp_provider.addFeatures(features)
        if not add_temp_ok:
          QgsMessageLog.logMessage(f"Failed add features to temporary layer for '{display_name}'.", plugin_tag, level=Qgis.Warning)
          return None
        ...
        if isinstance(returned_value_from_writer, tuple) and len(returned_value_from_writer) > 0:
            if isinstance(returned_value_from_writer[0], int):
                write_status_code_to_check = returned_value_from_writer[0]
            if len(returned_value_from_writer) > 1 and isinstance(returned_value_from_writer[1], str):
            ...
            QgsMessageLog.logMessage(f"Unexpected return type from writeAsVectorFormat for '{display_name}': {type(returned_value_from_writer)} value: {returned_value_from_writer}", plugin_tag, level=Qgis.Warning)
        ...
        if write_status_code_to_check == QgsVectorFileWriter.WriterError.NoError:
            ...
            if loaded_layer and loaded_layer.isValid():
                ...
                if loaded_node:
                    ...
                    QgsMessageLog.logMessage(f"Warning: Failed find layer node for saved layer '{display_name}'. Style/grouping may be incorrect.", plugin_tag, level=Qgis.Warning)
                    self.successfully_generated_layers.append(loaded_layer)
                    added_layer = loaded_layer
            else:
                QgsMessageLog.logMessage(f"Error: Failed to load saved layer back: '{uri_to_load}'. Layer reported as written successfully, but load failed. Check file and QGIS logs.", plugin_tag, level=Qgis.Critical)
        else: # Write failed
            error_type_str = f"WriterError (Code: {write_status_code_to_check})"
            error_msg_detail = error_message_from_writer if error_message_from_writer else "Details may be in GDAL/OGR logs or QGIS Message Log Panel."
            ...
            self.iface.messageBar().pushMessage(
                ...
                self.tr("Failed to write '{layer}'. Error code: {code}. Check logs.").format(layer=display_name, code=write_status_code_to_check),
                level=Qgis.Critical, duration=10
            )
      else: 
        QgsMessageLog.logMessage(f"Error: Unknown output_mode '{self.output_mode}' for layer '{display_name}'.", plugin_tag, level=Qgis.Critical)
    ...
    def _final_feedback(self, main_group: Optional[QgsLayerTreeGroup], root_node: QgsLayerTreeNode,
      ...
      if main_group is None and self.output_mode == 'memory': # If no group and memory, something is wrong
        self.iface.messageBar().pushMessage(self.tr("Error"), self.tr("Processing error: Main layer group not created."), level=Qgis.Critical)
        return
      ...
      if anything_successfully_generated:
        ...
        msg_parts.append(self.tr("{n} layer(s) generated. ").format(n=num_layers_created))
        ...
        self.iface.messageBar().pushMessage(self.tr("Success"), final_user_message, level=Qgis.Success, duration=10) # Increased duration
        QgsMessageLog.logMessage(final_log_message, PLUGIN_TAG, level=Qgis.Success) # Log overall success
        
        if main_group: # Only expand group if it exists (it might not if only file output and no group made)
          main_group.setExpanded(True)
      else:
        # This case means self.successfully_generated_layers is empty
        self.iface.messageBar().pushMessage(self.tr("Process Finished"), self.tr("No layers were generated. Check logs for warnings or errors."), level=Qgis.Warning, duration=7)
        QgsMessageLog.logMessage("Processing finished, but no layers were successfully generated or added.", PLUGIN_TAG, level=Qgis.Warning)
        if main_group and root_node.findGroup(main_group.name()): # Only try to remove group if it was created
    ...
    def create_runway_centreline_layer(self, point1: QgsPointXY, point2: QgsPointXY, runway_name: str,
        ...
        try:
          ...
          fields = QgsFields([
              ...
              QgsField("reciprocal_head", QVariant.Double, "Reciprocal Heading (degrees)", 10, 3),
              QgsField("toda", QVariant.String),
              QgsField("tora", QVariant.String),
              QgsField("lda", QVariant.String),
              QgsField("asda", QVariant.String),
              QgsField("arc_num", QVariant.String, "arc_num", 5), # ### NEW ###
              QgsField("arc_let", QVariant.String, "arc_let", 5)  # ### NEW ###
          ])
    ...
    def _generate_airport_wide_ols(self, processed_runway_data_list: List[dict],
        ...
        if ihs_base_geom:
          try:
            ...
            attr_map = {"rwy_name": self.tr("Airport Wide"), "surface": "IHS", "elev_m": IHS_ELEVATION_AMSL, "height_agl": ihs_base_height_agl, "ref_mos": ref_text, "shape_desc": "Convex Hull Approximation"}
                ...
                if temp_outer_geom and not temp_outer_geom.isEmpty():
                  ...
                  if temp_outer_geom.isGeosValid():
                    ...
                    if temp_conical_geom and not temp_conical_geom.isEmpty() and temp_conical_geom.isGeosValid():
                      ...
                      attr_map = {"rwy_name": self.tr("Airport Wide"), "surface": "Conical", "elev_m": conical_outer_elevation, "height_agl": conical_total_height_agl, "slope_perc": slope * 100.0, "ref_mos": ref, "height_extent": height_extent_agl}
                      for name, value in attr_map.items(): idx = fields.indexFromName(name);
                      if idx != -1: feature.setAttribute(idx, value)
                  ...
                  if outer_geom.isGeosValid():
                    ...
                    if previous_contour_geom and previous_contour_geom.isGeosValid():
                      ...
                      if ring_geom and not ring_geom.isEmpty() and ring_geom.isGeosValid():
                        ...
                        attr_map = { "surface": f"Conical Contour {current_target_contour_elev_amsl:.0f}m", "contour_elev_am": current_target_contour_elev_amsl, "contour_hgt_abv": contour_h_above_ihs, "ref_mos": ref }
                        for name, value in attr_map.items(): idx = fields.indexFromName(name);
                        if idx != -1: feature.setAttribute(idx, value)
        ...
        if ohs_params:
          ...
          if radius is not None and height_agl is not None and radius > 0:
            ...
            if found_arp_point_layer:
              ...
              if arp_feat and arp_feat.hasGeometry() and not arp_feat.geometry().isNull():
                ...
                QgsMessageLog.logMessage(
                  ...
                  PLUGIN_TAG, Qgis.Info
                )
                
                acceptable_point_wkb_types = {
                  QgsWkbTypes.Point, QgsWkbTypes.PointZ, QgsWkbTypes.PointM, QgsWkbTypes.PointZM,
                  QgsWkbTypes.MultiPoint, QgsWkbTypes.MultiPointZ, QgsWkbTypes.MultiPointM, QgsWkbTypes.MultiPointZM
                }
                ...
                if actual_wkb_type in acceptable_point_wkb_types:
                  ...
                  # --- TEMPORARY BYPASS FOR GPKG ARP POLYGON ISSUE ---
                  if self.output_mode == 'file' and self.output_format_extension.lower() == '.gpkg':
                    QgsMessageLog.logMessage("TEMP BYPASS: GPKG output detected, and ARP geom is not point. Skipping OHS point extraction. OHS will likely fail or be incorrect.", PLUGIN_TAG, Qgis.Warning)
                    OHS_GENERATION_BYPASSED_DUE_TO_ARP_GEOM = True
                    arp_point_xy = None # Ensure it's None so OHS block is skipped cleanly
                  # --- END TEMPORARY BYPASS ---
              else:
                QgsMessageLog.logMessage(f"ARP layer '{found_arp_point_layer.name()}' found, but no valid features/geometry for OHS.", PLUGIN_TAG, level=Qgis.Warning)
            else:
              QgsMessageLog.logMessage(f"Could not find a valid POINT layer named '{expected_arp_layer_name}' for OHS.", PLUGIN_TAG, level=Qgis.Warning)
            ...
            if OHS_GENERATION_BYPASSED_DUE_TO_ARP_GEOM:
              QgsMessageLog.logMessage("OHS generation for GPKG bypassed due to ARP geometry issue.", PLUGIN_TAG, level=Qgis.Info)
            # --- END TEMPORARY BYPASS CHECK ---
            elif arp_point_xy: # Only proceed if arp_point_xy was successfully set AND NOT BYPASSED
              try:
                ...
                if ohs_full_circle_geom and not ohs_full_circle_geom.isEmpty():
                  ...
                  if ohs_full_circle_geom.isGeosValid():
                    ...
                    attr_map = { "surface": "OHS", "elev_m": ohs_elevation_amsl, "height_agl": height_agl, "ref_mos": ref, "radius_m": radius }
      ...
      for runway_data in processed_runway_data_list:
          ...
          for side_label, strip_edge, outward_azimuth in [('L', strip_edge_l, rwy_params['azimuth_perp_l']), ('R', strip_edge_r, rwy_params['azimuth_perp_r'])]:
                ...
                if poly_geom:
                  ...
                  attr_map = {"rwy_name": runway_name, "surface": "Transitional", "section_desc": "Strip Side", "side": side_label, "slope_perc": transitional_slope * 100.0, "ref_mos": transitional_ref}
                      ...
                      if clipped_segment:
                          ...
                          if pa_upper_start and pa_upper_end:
                              ...
                              if poly_geom:
                                  ...
                                  attr_map = {"rwy_name": runway_name, "surface": "Transitional", "end_desig": end_desig, "section_desc": f"Approach Sec {i+1}", "side": side_label, "slope_perc": transitional_slope * 100.0, "ref_mos": transitional_ref}
                                  for name, value in attr_map.items(): idx = transitional_fields.indexFromName(name);
                                  if idx != -1: feat.setAttribute(idx, value)
                                  transitional_features.append(feat)
                              # else: Failure logged by helper
    ...
    def process_met_station_surfaces(self, met_point_proj_crs: QgsPointXY, icao_code: str, target_crs: QgsCoordinateReferenceSystem, layer_group: QgsLayerTreeGroup) -> Tuple[bool, List[QgsVectorLayer]]:
        ...
        # Layer 1: Point
        try:
            fields = QgsFields([QgsField("name", QVariant.String), QgsField("coord_east", QVariant.Double), QgsField("coord_north", QVariant.Double)])
    ...
    def process_guideline_b(self, runway_data: dict, layer_group: QgsLayerTreeGroup) -> bool:
        ...
        fields = QgsFields([QgsField("rwy_name", QVariant.String), QgsField("zone", QVariant.String), QgsField("end_desig", QVariant.String)])
        features_to_add = []
        ...
        layer_created = self._create_and_add_layer("Polygon", f"WSZ_{runway_name.replace('/', '_')}", f"WSZ {self.tr('RWY')} {runway_name}", fields, features_to_add, layer_group, "WSZ Runway")
...
# ============================================================
    def _get_conical_contour_fields(self) -> QgsFields:
      ...
      fields = QgsFields([
        ...
        QgsField("contour_hgt_abv", QVariant.Double, self.tr("Height Above IHS (m)"), 10, 2),
    ...
    def _get_ols_fields(self, surface_type: str) -> QgsFields:
      ...
      fields_list = [
        ...
        QgsField("elev_m", QVariant.Double, self.tr("Outer Elev (AMSL)"), 10, 2), # Clarify: Elevation at outer edge of this section
      ...
      if surface_type in ["Approach", "TOCS", "InnerApproach"]:
        fields_list.extend([
          QgsField("len_m", QVariant.Double, self.tr("Section Length (m)"), 12, 2), # Clarify: Length of this section
      ...
      remove_map = {
        "IHS": ["end_desig", "len_m", "innerw_m", "outerw_m", "diverg_perc", "origin_offset", "height_extent", "radius_m", "side", "slope_perc"],
        "Conical": ["end_desig", "len_m", "innerw_m", "outerw_m", "diverg_perc", "origin_offset", "shape_desc", "radius_m", "side"],
        "OHS": ["end_desig", "len_m", "innerw_m", "outerw_m", "diverg_perc", "origin_offset", "shape_desc", "height_extent", "side", "slope_perc"],
        "Transitional": ["end_desig", "len_m", "innerw_m", "outerw_m", "diverg_perc", "origin_offset", "shape_desc", "height_extent", "radius_m"],
      ...
      for field in fields_list:
          if field.name() not in fields_to_remove:
              ...
              if field.name() == "elev_m": field.setAlias(self.tr("Section Outer Elev (AMSL)"))
              elif field.name() == "height_agl": field.setAlias(self.tr("Section Height Gain (m)"))
              elif field.name() == "len_m": field.setAlias(self.tr("Section Length (m)"))
                ...
                if cl_point and half_width > 0:
                    ...
                    if pt_l and pt_r:
                        ...
                        if contour_geom and not contour_geom.isEmpty():
                            ...
                            contour_attr_map = { "rwy_name": runway_data.get('short_name', 'N/A'), "end_desig": end_desig, "surface": f"Approach Contour {target_elev:.0f}m", "contour_elev_am": target_elev, "ref_mos": ref }
                 ...
                 if start_point_p:
                      ...
                      if inner_app_geom_p and not inner_app_geom_p.isEmpty() and inner_app_geom_p.isGeosValid():
                           ...
                           attr_map = {
                               ...
                               "elev_m": outer_elev_p, "height_agl": height_agl_p,
                               "slope_perc": slope_p * 100.0, "ref_mos": inner_app_ref,
                               "len_m": inner_app_length, "innerw_m": inner_app_width,
                 ...
                 if start_point_r:
                      ...
                      if inner_app_geom_r and not inner_app_geom_r.isEmpty() and inner_app_geom_r.isGeosValid():
                           ...
                           attr_map = {
                               ...
                               "elev_m": outer_elev_r, "height_agl": height_agl_r,
                               "slope_perc": slope_r * 100.0, "ref_mos": inner_app_ref,
                               "len_m": inner_app_length, "innerw_m": inner_app_width,
    ...
    def process_guideline_i(self, runway_data: dict, layer_group: QgsLayerTreeGroup) -> bool:
        ...
        layer_created = self._create_and_add_layer("Polygon", f"PSA_{runway_name.replace('/', '_')}", f"PSA {self.tr('RWY')} {runway_name}", fields, features_to_add, layer_group, "PSA Runway")
        return layer_created is not None
...
# ============================================================
    ...
    def _get_taxiway_separation_fields(self) -> QgsFields:
      ...
      fields = QgsFields([
        ...
        QgsField("desc", QVariant.String, self.tr("desc"), 100), # Renamed Alias, updated length
        QgsField("offset_m", QVariant.Double, self.tr("offset_m"), 10, 2),
        QgsField("ref_mos", QVariant.String, self.tr("MOS Reference"), 100), # Renamed field and Alias
        QgsField("appr_type", QVariant.String, self.tr("appr_type"), 50), # New field
        QgsField("arc_num", QVariant.String, self.tr("arc_num"), 5), # New field
        QgsField("arc_let", QVariant.String, self.tr("arc_let"), 5), # New field
        QgsField("side", QVariant.String, self.tr("Side (L/R)"), 5)
      ])
      return fields
