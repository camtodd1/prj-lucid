...
MOST_LIMITING_OLS_SAMPLE_SPACING_M = 100.0
MOST_LIMITING_OLS_MAX_SAMPLE_POINTS = 120000
...
class SafeguardingBuilder:
    ...
    def run_safeguarding_processing(self):
        ...
        if runway_input_list:
            ...
            if guideline_groups.get("F") is not None:
                try:
                    most_limiting_created = self.generate_most_limiting_ols_samples(
                        guideline_groups["F"],
                        icao_code,
                        target_crs,
                    )
                    if most_limiting_created:
                        any_guideline_processed_ok = True
                except Exception as e_limit:
                    QgsMessageLog.logMessage(
                        f"Critical Error generating most limiting OLS samples: {e_limit}\n{traceback.format_exc()}",
                        plugin_tag,
                        level=Qgis.Critical,
                    )
    ...
    def _attribute_as_float(self, value: Any) -> Optional[float]:
        """Converts a feature attribute to float when it represents a numeric value."""
        if value is None:
            return None
        try:
            return float(value)
    ...
    def generate_most_limiting_ols_samples(
        self,
        layer_group: QgsLayerTreeGroup,
        icao_code: str,
        target_crs: QgsCoordinateReferenceSystem,
        ...
        """
        Samples generated OLS polygon layers and creates a point layer showing the
        lowest controlling elevation at each covered grid point.

        This is a pragmatic OLS envelope: it uses each source feature's elev_m
        attribute rather than recalculating a continuous surface equation.
        """
        plugin_tag = PLUGIN_TAG
        spacing = MOST_LIMITING_OLS_SAMPLE_SPACING_M
        candidates: List[Dict[str, Any]] = []
        min_x = min_y = max_x = max_y = None

        for layer in list(self.successfully_generated_layers):
            if layer is None or not layer.isValid():
                continue
            ...
            if "OLS" not in layer_name:
                continue
            if "Contour" in layer_name or "Most Limiting" in layer_name:
                continue

            field_names = {field.name() for field in layer.fields()}
            if "elev_m" not in field_names:
                continue

            for source_feature in layer.getFeatures():
                elev_m = self._attribute_as_float(source_feature.attribute("elev_m"))
                if elev_m is None:
                    continue

                source_geom = source_feature.geometry()
                source_geom = self._ensure_valid_geometry(
                    source_geom,
                    f"most limiting OLS source feature {source_feature.id()} in {layer_name}",
                )
                ...
                min_x = bbox.xMinimum() if min_x is None else min(min_x, bbox.xMinimum())
                min_y = bbox.yMinimum() if min_y is None else min(min_y, bbox.yMinimum())
                max_x = bbox.xMaximum() if max_x is None else max(max_x, bbox.xMaximum())
                max_y = bbox.yMaximum() if max_y is None else max(max_y, bbox.yMaximum())

                def attr_text(field_name: str) -> str:
                ...
                candidates.append(
                    {
                        ...
                        "bbox": bbox,
                        "elev_m": elev_m,
                        "surface": attr_text("surface"),
                        "rwy_name": attr_text("rwy_name"),
                        "end_desig": attr_text("end_desig"),
                        "source_layer": layer_name,
                        "source_fid": str(source_feature.id()),
                    }
                )

        if not candidates or min_x is None or min_y is None or max_x is None or max_y is None:
            QgsMessageLog.logMessage(
                "Most limiting OLS samples skipped: no generated OLS polygon features with numeric elev_m were found.",
                plugin_tag,
                level=Qgis.Info,
            )
            return False
        ...
        if width <= 0 or height <= 0:
            QgsMessageLog.logMessage(
                "Most limiting OLS samples skipped: combined OLS extent is empty.",
                plugin_tag,
                level=Qgis.Warning,
            )
            return False

        estimated_points = (math.floor(width / spacing) + 1) * (
            math.floor(height / spacing) + 1
        )
        if estimated_points > MOST_LIMITING_OLS_MAX_SAMPLE_POINTS:
            spacing *= math.sqrt(estimated_points / MOST_LIMITING_OLS_MAX_SAMPLE_POINTS)
            estimated_points = (math.floor(width / spacing) + 1) * (
                math.floor(height / spacing) + 1
            )
            QgsMessageLog.logMessage(
                f"Most limiting OLS sample grid coarsened to {spacing:.1f}m to keep estimated points near {estimated_points}.",
                plugin_tag,
                level=Qgis.Warning,
            )

        fields = QgsFields()
        fields.append(QgsField("limit_elev_m", QVariant.Double))
        ...
        fields.append(QgsField("source_fid", QVariant.String))
        fields.append(QgsField("candidate_count", QVariant.Int))
        fields.append(QgsField("sample_x", QVariant.Double))
        fields.append(QgsField("sample_y", QVariant.Double))
        fields.append(QgsField("spacing_m", QVariant.Double))
        fields.append(QgsField("eval_method", QVariant.String))

        sample_features: List[QgsFeature] = []
        y = min_y
        while y <= max_y + 1e-9:
            x = min_x
            while x <= max_x + 1e-9:
                ...
                for candidate in candidates:
                    ...
                    if not (
                        candidate_geom.contains(point_geom)
                        or candidate_geom.intersects(point_geom)
                    ):
                        continue
                    ...
                    if (
                        ...
                        or candidate["elev_m"] < best_candidate["elev_m"]
                    ):
                        best_candidate = candidate

                if best_candidate is not None:
                    sample_feature = QgsFeature(fields)
                    sample_feature.setGeometry(point_geom)
                    sample_feature.setAttributes(
                        [
                            best_candidate["elev_m"],
                            best_candidate["surface"],
                            best_candidate["rwy_name"],
                            best_candidate["end_desig"],
                            best_candidate["source_layer"],
                            best_candidate["source_fid"],
                            candidate_count,
                            x,
                            y,
                            spacing,
                            "feature_elev_m_sample",
                        ]
                    )
        ...
        if not sample_features:
            QgsMessageLog.logMessage(
                "Most limiting OLS samples skipped: grid did not intersect generated OLS polygons.",
                plugin_tag,
                level=Qgis.Warning,
            )
            return False
        ...
        layer = self._create_and_add_layer(
            "Point",
            f"OLS_MostLimitingSamples_{icao_code}",
            f"{self.tr('OLS')} Most Limiting Samples {icao_code}",
            fields,
            sample_features,
            layer_group,
            "Default Point",
        ...
        QgsMessageLog.logMessage(
            f"Generated most limiting OLS sample layer with {sample_count} points from {len(candidates)} source features at {spacing:.1f}m spacing.",
            plugin_tag,
            level=Qgis.Info,
        )
        return True
