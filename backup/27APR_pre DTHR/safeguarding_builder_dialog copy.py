...
class RunwayWidgetGroup(QtWidgets.QGroupBox):
    ...
    def __init__(self, index: int, coord_validator: QtGui.QValidator, parent: QtWidgets.QWidget = None):
        """Constructor."""
        # Use the index to create the GroupBox title string BEFORE calling super
        title = f"Runway {index}"
        super().__init__(title, parent) # Call QGroupBox constructor correctly

        # Assign instance variables AFTER super() call
        self.index = index
        # Use a unique numeric validator for this instance if needed, or reuse one passed in
        ...
        # Store the passed-in coordinate validator
        ...
        # Setup UI specific to this runway group
        self._setup_ui()
        # Connect signals specific to this runway group's widgets
    ...
    def _setup_ui(self):
        ...
        # <<< NEW >>> Label for Displaced Threshold
        ...
        # <<< NEW >>> Displaced Threshold LineEdits
        ...
        self.thr_displaced_1_le.setToolTip("Displaced threshold distance for primary end (meters). Leave blank if none.")
        ...
        self.thr_displaced_2_le.setToolTip("Displaced threshold distance for reciprocal end (meters). Leave blank if none.")
        # Validator for non-negative distance
        displaced_validator = QtGui.QDoubleValidator(0.0, 9999.9, 1, self) # Min 0.0, Max 9999.9, 1 decimal
        displaced_validator.setNotation(QtGui.QDoubleValidator.Notation.StandardNotation)
        self.thr_displaced_1_le.setValidator(displaced_validator)
        self.thr_displaced_2_le.setValidator(displaced_validator)
        ...
        # <<< NEW >>> Row 4: Displaced Threshold
        gridLayout_Coords.addWidget(label_displaced_row, 4, 0); gridLayout_Coords.addWidget(self.thr_displaced_1_le, 4, 1); gridLayout_Coords.addWidget(self.thr_displaced_2_le, 4, 2)
    ...
    def _connect_signals(self):
        ...
        self.thr_displaced_1_le.textChanged.connect(self.inputChanged.emit) # <<< NEW FIELD >>>
        self.thr_displaced_2_le.textChanged.connect(self.inputChanged.emit) # <<< NEW FIELD >>>
    ...
    def get_input_data(self) -> Dict[str, str]:
            ...
            data = {
                    ...
                    "thr_displaced_1": self.thr_displaced_1_le.text(), # <<< NEW FIELD >>>
                    "thr_displaced_2": self.thr_displaced_2_le.text(), # <<< NEW FIELD >>>
    ...
    def set_input_data(self, data: Dict[str, str]):
        ...
        widgets_to_block = [
            ...
            self.thr_displaced_1_le, self.thr_displaced_2_le, # <<< NEW FIELDS >>>
        ...
        try:
            ...
            if self.thr_displaced_1_le: self.thr_displaced_1_le.setText(data.get("thr_displaced_1", "")) # <<< NEW FIELD >>>
            if self.thr_displaced_2_le: self.thr_displaced_2_le.setText(data.get("thr_displaced_2", "")) # <<< NEW FIELD >>>
    ...
    # --- update_display_labels --- (No changes needed here for displaced threshold)
    ...
    def clear_inputs(self):
        ...
        try:
            ...
            if self.thr_displaced_1_le: self.thr_displaced_1_le.clear() # <<< NEW FIELD >>>
            if self.thr_displaced_2_le: self.thr_displaced_2_le.clear() # <<< NEW FIELD >>>
...
class SafeguardingBuilderDialog(QtWidgets.QDialog, FORM_CLASS):
    ...
    # <<< CORRECT __init__ for SafeguardingBuilderDialog >>>
    def __init__(self, parent=None):
        """Constructor."""
        super().__init__(parent)
        self.setupUi(self) # Load UI elements defined in the .ui file

        # Initialize instance variables for dialog state
        ...
        # Disable add button if layout is missing
        ...
        # Define validators used by this dialog and its child groups
        ...
        # Apply validators to the global ARP/MET fields
        ...
        # Check layout again before adding
        ...
        # --- Ensure initial size calculation happens after setup ---
        QtCore.QTimer.singleShot(0, self._update_dialog_height)
    # <<< END CORRECT __init__ for SafeguardingBuilderDialog >>>

    # =========================================================================
    # == Initialization Helper Methods (mostly unchanged)
    # =========================================================================
    # ... (methods _setup_arp_validators, _connect_global_controls, _setup_cns_manual_entry remain the same) ...
    ...
    # =========================================================================
    # ... (methods _get_next_runway_id, update_all_runway_calculations, update_runway_calculations,
    #      add_runway_group, remove_runway_group, _remove_runway_group_internal, _update_dialog_height remain the same) ...
    ...
    def update_runway_calculations(self, runway_index: int):
        ...
        try:
            ...
            try: # Inner try for designation math
                ...
                full_desig_1_str = f"RWY {rwy_desig_val:02d}{rwy_suffix}"
                type1_label_str = f"{full_desig_1_str} Approach Type:"
                ...
                full_desig_2_str = f"RWY {rec_desig_num_str}{rec_suffix}"
                type2_label_str = f"{full_desig_2_str} Approach Type:"
    ...
    def get_all_input_data(self) -> Optional[Dict[str, Any]]:
        ...
        if icao is None: # Indicates critical failure in global inputs
            return None # Message box already shown in _get_global_inputs

        # Add validated global data to the dictionary
        final_data.update({
            'icao_code': icao,
            'arp_point': arp_pt,
            'arp_easting': arp_e,
        ...
        if not self._runway_groups:
            ...
            for index, group_widget in sorted(self._runway_groups.items()):
                ...
                if validated_runway:
                    # Ensure keys exist (although validator should add them)
                    if 'thr_elev_1' not in validated_runway: validated_runway['thr_elev_1'] = None
                    if 'thr_elev_2' not in validated_runway: validated_runway['thr_elev_2'] = None
                    if 'thr_displaced_1' not in validated_runway: validated_runway['thr_displaced_1'] = None # <<< NEW KEY >>>
                    if 'thr_displaced_2' not in validated_runway: validated_runway['thr_displaced_2'] = None # <<< NEW KEY >>>
    ...
    def _validate_runway_data(self, index: int, inputs: Dict[str, str], errors: List[str]) -> Optional[Dict[str, Any]]:
        ...
        # <<< NEW VALIDATION >>> Validate Displaced Thresholds (Optional, non-negative)
        try: # Primary Displaced
            disp1_str = inputs.get('thr_displaced_1', '').strip()
            if disp1_str:
                disp1_val = float(disp1_str)
                if disp1_val < 0: raise ValueError("Displaced threshold cannot be negative")
                validated['thr_displaced_1'] = disp1_val
            else:
                validated['thr_displaced_1'] = None # Explicitly None if empty
        except ValueError:
            errors.append(f"Rwy {index}: Invalid primary displaced threshold '{inputs.get('thr_displaced_1', '')}'. Must be non-negative.")
            current_errors += 1
            validated['thr_displaced_1'] = None # Ensure key exists even on error

        try: # Reciprocal Displaced
            disp2_str = inputs.get('thr_displaced_2', '').strip()
            if disp2_str:
                disp2_val = float(disp2_str)
                if disp2_val < 0: raise ValueError("Displaced threshold cannot be negative")
                validated['thr_displaced_2'] = disp2_val
            else:
                validated['thr_displaced_2'] = None
        except ValueError:
            errors.append(f"Rwy {index}: Invalid reciprocal displaced threshold '{inputs.get('thr_displaced_2', '')}'. Must be non-negative.")
            current_errors += 1
            validated['thr_displaced_2'] = None
        ...
        try:
            ...
            current_errors += 1
            validated['width'] = None # Ensure key exists even on error
        ...
        try:
            ...
            current_errors += 1
            validated['shoulder'] = None # Ensure key exists even on error
            ...
            if arp_elev_lineEdit:
                ...
                if arp_elev_str:
                    try:
                        ...
                        # QgsMessageLog.logMessage(f"ARP Elevation read: {arp_elev}", DIALOG_LOG_TAG, level=Qgis.Info)
                    except ValueError:
                        # CRITICAL ERROR if elevation is entered but invalid format
                ...
                # else: Empty elevation is acceptable (arp_elev remains None)
            # else: Elevation widget not found (arp_elev remains None)
        ...
        for row in range(total_rows):
            ...
            try:
                ...
                if isinstance(combo_box, QComboBox) and combo_box.currentIndex() > 0 and combo_box.currentText():
                    ...
                    valid_row = False # Type not selected or combo box missing

                # Validate Coordinates (Must be present and valid numbers)
                if valid_row:
                    ...
                    if not x_str or not y_str:
                        valid_row = False # Missing coordinates
                    else:
                        try:
                            ...
                            if point_geom_project_crs.isNull():
                                valid_row = False; error_in_row = True; QgsMessageLog.logMessage(f"CNS Row {row+1}: Null geom.", DIALOG_LOG_TAG, level=Qgis.Warning)
                        except ValueError:
                            valid_row = False; error_in_row = True; QgsMessageLog.logMessage(f"CNS Row {row+1}: Invalid coords.", DIALOG_LOG_TAG, level=Qgis.Warning)

                # Validate Elevation (Optional, but must be valid number if present)
                if valid_row:
                    elev_item = cns_table.item(row, 3); elev_str = elev_item.text().strip() if elev_item else ""
                    if elev_str:
                        try: facility_elevation = float(elev_str)
                        except ValueError:
                             facility_elevation = None; error_in_row = True # Don't make row invalid, just flag error & ignore elev
                             QgsMessageLog.logMessage(f"CNS Row {row+1}: Invalid elev, ignoring elevation.", DIALOG_LOG_TAG, level=Qgis.Warning)

            except Exception as e:
                 valid_row = False; error_in_row = True; QgsMessageLog.logMessage(f"CNS Row {row+1} Error: {e}", DIALOG_LOG_TAG, level=Qgis.Critical)

            # --- Append if Row is Fundamentally Valid ---
            if valid_row and point_geom_project_crs:
                # Generate a basic ID
                facility_id = f'Manual_{row+1}_{facility_type.replace(" ", "_").replace("-","").replace("(","").replace(")","")}'[:50]
                cns_facilities_data.append({
                    'id': facility_id,
                    'type': facility_type,
                    'geom': point_geom_project_crs, # Geometry in Project CRS
                 ...
                 rows_with_errors += 1
                 skipped_rows += 1
            elif not valid_row: # Skipped due to missing required data (type or coords)
                 skipped_rows += 1

        # --- Feedback ---
        if rows_with_errors > 0:
             QMessageBox.warning(self, "CNS Data Warning",
                                 f"{rows_with_errors} CNS row(s) had errors (e.g., invalid coordinates or elevation) and were skipped or had data ignored.\n"
                                 f"({skipped_rows} total rows skipped or incomplete). Check the QGIS Log Messages Panel for details.")
        elif skipped_rows > 0 and skipped_rows == total_rows and total_rows > 0:
             QMessageBox.information(self, "CNS Data Info", "All CNS rows skipped (missing required type or coordinate data).")
        elif skipped_rows > 0:
             QMessageBox.information(self, "CNS Data Info", f"{skipped_rows} CNS rows skipped (missing required type or coordinate data).")
    ...
    # =========================================================================
    # ... (methods clear_all_inputs, save_input_data, load_input_data remain the same,
    #      as they rely on get/set methods which were updated) ...
    def clear_all_inputs(self, confirm: bool = True) -> None:
        ...
        # Remove ALL Runway Groups (using internal method without confirmation)
        indices_to_remove = list(self._runway_groups.keys())
        for index in indices_to_remove:
            self._remove_runway_group_internal(index) # Does the actual removal and cleanup

        # Reset Internal State
        self._runway_groups.clear() # Should be empty now, but make sure
        self._runway_id_counter = 0 # Reset the counter

        # Add back one empty runway group
        if self.scroll_area_layout:
            self.add_runway_group() # This will add a new group with the next ID (1)
        else:
            QgsMessageLog.logMessage("Warn (Clear): Layout missing, couldn't add initial runway group back.", DIALOG_LOG_TAG, level=Qgis.Warning)

        self._update_dialog_height() # Adjust dialog size if needed

    def save_input_data(self):
        """Gathers current inputs and saves them to a JSON file."""
        # Get suggested filename based on ICAO code
        icao_le = getattr(self, 'lineEdit_airport_name', self.findChild(QtWidgets.QLineEdit, "lineEdit_airport_name"))
        icao_code = icao_le.text().strip().upper() if icao_le else ""
        ...
        if not file_path.lower().endswith(".json"): file_path += ".json" # Ensure extension

        data_to_save = {}

        # --- Save Global Data (Read directly from widgets) ---
        ...
        # --- Save Runway Data (Use get_input_data from helper class) ---
        # Uses the updated get_input_data which includes displaced threshold fields
        data_to_save["runways"] = [self._runway_groups[idx].get_input_data() for idx in sorted(self._runway_groups.keys())]

        # --- Save CNS Data (Read directly from table) ---
        cns_list = []
        cns_table = getattr(self, 'table_cns_facility', self.findChild(QtWidgets.QTableWidget, 'table_cns_facility'))
        if cns_table:
            for row in range(cns_table.rowCount()):
                try:
                    ...
                    # Save even if incomplete, load process will handle it
                    type_txt = combo.currentText() if isinstance(combo, QComboBox) else "" # Save combo text directly
                    x_item, y_item, elev_item = cns_table.item(row, 1), cns_table.item(row, 2), cns_table.item(row, 3)
                    x_txt = x_item.text() if x_item else ""
                    y_txt = y_item.text() if y_item else ""
                    elev_txt = elev_item.text() if elev_item else ""
                    cns_list.append({
                        "type": type_txt,
                        "easting_x": x_txt,
                        "northing_y": y_txt,
    ...
    def load_input_data(self):
        ...
        # --- Check if overwriting data ---
        has_data = False
        global_widgets = [
            getattr(self, name, self.findChild(QtWidgets.QLineEdit, name))
            for name in ["lineEdit_airport_name", "lineEdit_arp_easting", "lineEdit_arp_northing", "lineEdit_arp_elevation", "lineEdit_met_easting", "lineEdit_met_northing"]
        ]
        if any(w and w.text() for w in global_widgets if w is not None): has_data = True
        ...
        if has_data:
            reply = QtWidgets.QMessageBox.question(self, self.tr('Confirm Load'),
        ...
        try:
            ...
            # Check for essential keys, allow others to be missing for flexibility
            required_keys = ["icao_code", "runways"] # ARP/MET/CNS are optional in the file itself
            missing_keys = [k for k in required_keys if k not in loaded_data]
            if missing_keys: raise ValueError(f"Missing required keys: {', '.join(missing_keys)}")
            ...
            # --- Clear Existing Data (no confirmation needed now) ---
            self.clear_all_inputs(confirm=False) # This adds one empty runway group back
            ...
            if icao_le: icao_le.setText(loaded_data.get("icao_code", "")) # Required key, should exist
            if arp_east_le: arp_east_le.setText(loaded_data.get("arp_easting", "")) # Optional key
            if arp_north_le: arp_north_le.setText(loaded_data.get("arp_northing", "")) # Optional key
            if arp_elev_le: arp_elev_le.setText(loaded_data.get("arp_elevation", "")) # Optional key
            if met_east_le: met_east_le.setText(loaded_data.get("met_easting", "")) # Optional key
            if met_north_le: met_north_le.setText(loaded_data.get("met_northing", "")) # Optional key


            # --- Populate Runway Fields ---
            # Uses the updated set_input_data which includes displaced threshold fields
            loaded_runways_list = loaded_data.get("runways", []) # Should exist due to validation
            if loaded_runways_list:
                ...
                if first_group:
                    ...
                    # Ensure all expected keys are present in the dict passed to set_input_data
                    # Use defaults if keys are missing from older JSON files
                    first_runway_data.setdefault("thr_displaced_1", "")
                    first_runway_data.setdefault("thr_displaced_2", "")
                    first_group.set_input_data(first_runway_data) # Populates and triggers update
                else:
                ...
                for runway_data_item in loaded_runways_list[1:]:
                    try:
                        self.add_runway_group() # Adds widget, stores in dict, increments counter
                        new_index = self._runway_id_counter # Get the index of the newly added group
                        new_group = self._runway_groups.get(new_index)
                        if new_group:
                            # Ensure all expected keys are present
                            runway_data_item.setdefault("thr_displaced_1", "")
            ...
            elif self._runway_groups: # File has no runways, update the one empty group's labels
            ...
            # --- Populate CNS Table ---
            cns_table = getattr(self, 'table_cns_facility', self.findChild(QtWidgets.QTableWidget, 'table_cns_facility'))
            loaded_cns_list = loaded_data.get("cns_facilities", []) # Optional key
            if cns_table and loaded_cns_list:
                # Table should be empty from clear_all_inputs
                cns_table.setRowCount(0)
                for item_data in loaded_cns_list:
                   if not isinstance(item_data, dict):
                       QgsMessageLog.logMessage(f"Load CNS Warning: Skipping non-dictionary item: {item_data}", DIALOG_LOG_TAG, level=Qgis.Warning)
                       continue
                   try:
                       ...
                       combo = QComboBox()
                       combo.addItems([""] + self.CNS_FACILITY_TYPES) # Assumes CNS_FACILITY_TYPES is populated
                       type_str = item_data.get("type", "") # Get saved type
                       idx = combo.findText(type_str, QtCore.Qt.MatchFlag.MatchFixedString)
                       combo.setCurrentIndex(idx if idx >= 0 else 0) # Select if found, else default to blank
                       cns_table.setCellWidget(row, 0, combo)

                       # Set Easting/X, Northing/Y, Elevation
                   ...
                   except Exception as e: QgsMessageLog.logMessage(f"Load CNS error processing item {item_data}: {e}", DIALOG_LOG_TAG, level=Qgis.Warning)

            # --- Final Steps ---
            self._update_dialog_height() # Adjust size after adding elements
            QMessageBox.information(self, self.tr("Load Successful"), self.tr("Input data loaded from:\n{path}").format(path=file_path))

        except (IOError, json.JSONDecodeError, ValueError) as e:
            # Handle file read, JSON parsing, or validation errors
            error_details = f"{type(e).__name__}: {e}"
            log_msg = f"Load Error ({file_path}): {error_details}"
            user_msg = self.tr("Error loading data from file:") + f"\n{file_path}\n\n{self.tr('Error')}: {error_details}"
            QgsMessageLog.logMessage(log_msg, DIALOG_LOG_TAG, level=Qgis.Critical)
            QMessageBox.critical(self, self.tr("Load Error"), user_msg)
            # Attempt to clear potentially partially loaded data
            ...
            # Handle unexpected errors during loading/populating
            error_details = f"{type(e).__name__}: {e}"
            log_msg = f"Unexpected load error ({file_path}): {error_details}"
            user_msg = self.tr("An unexpected error occurred during loading:") + f"\n\n{self.tr('Error')}: {error_details}"
            QgsMessageLog.logMessage(log_msg, DIALOG_LOG_TAG, level=Qgis.Critical)
            QMessageBox.critical(self, self.tr("Load Error"), user_msg)
            # Attempt to clear potentially partially loaded data
            try: self.clear_all_inputs(confirm=False)
...
if __name__ == '__main__':
    ...
    class MockQgsProject:
        def __init__(self): self._crs = QgsCoordinateReferenceSystem("EPSG:4326") # Default mock CRS
        def instance(self): return self
        def crs(self): return self._crs
    if not QgsProject.instance(): QgsProject.setInstance(MockQgsProject()) # Use setInstance if available, else monkey patch
    # --- End Mock ---
