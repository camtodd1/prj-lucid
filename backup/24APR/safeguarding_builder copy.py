...
class SafeguardingBuilder:
    ...
    def run_safeguarding_processing(self):
        ...
        # Define Style Map
        self.style_map = { "ARP": "arp_point.qml", "Runway Centreline": "rwy_centreline_line.qml", "MET Station Location": "default_point.qml", "MET Instrument Enclosure": "default_zone_polygon.qml", "MET Buffer Zone": "default_zone_polygon.qml", "MET Obstacle Buffer Zone": "default_zone_polygon.qml", "Runway Pavement": "physical_runway.qml", "Runway Shoulders": "physical_shoulders.qml", "Runway Graded Strips": "physical_graded_strips.qml", "Runway Overall Strips": "physical_overall_strips.qml", "Runway End Safety Areas (RESA)": "physical_resa.qml", "Stopways": "physical_stopway.qml", "WSZ Runway": "guideline_b_wsz.qml", "WMZ A": "default_zone_polygon.qml", "WMZ B": "default_zone_polygon.qml", "WMZ C": "default_zone_polygon.qml", "LCZ A": "default_zone_polygon.qml", "LCZ B": "default_zone_polygon.qml", "LCZ C": "default_zone_polygon.qml", "LCZ D": "default_zone_polygon.qml", "CNS Circle Zone": "default_cns_zone_polygon.qml", "CNS Donut Zone": "default_cns_zone_polygon.qml", "Default CNS": "default_cns_zone_polygon.qml", "PSA Runway": "guideline_i_psa.qml", "Default Polygon": "default_zone_polygon.qml", "Default Line": "default_line.qml", "Default Point": "default_point.qml" }
    ...
    def generate_physical_geometry(self, runway_data: dict) -> Optional[List[Tuple[str, QgsGeometry, dict]]]:
       ...
       thr_point = runway_data.get('thr_point'); rec_thr_point = runway_data.get('rec_thr_point')
       runway_width = runway_data.get('width'); runway_name = runway_data.get('short_name', 'RWY')
       log_name = runway_name if runway_name != 'RWY' else f"RWY_{runway_data.get('original_index','?')}"

       # --- Add This Debug Log ---
       QgsMessageLog.logMessage(f"DEBUG generate_physical_geometry - Received runway_data: {runway_data}", PLUGIN_TAG, Qgis.Info)
       # --- End Debug Log ---
       ...
       if params is None:
           QgsMessageLog.logMessage(f"Skipping physical geom {log_name}: Failed runway params.", PLUGIN_TAG, level=Qgis.Warning)
           return None

       # --- Log the points specifically ---
       QgsMessageLog.logMessage(f"DEBUG generate_physical_geometry for {log_name} - thr_point type: {type(thr_point)}, value: {thr_point}", PLUGIN_TAG, Qgis.Info)
       QgsMessageLog.logMessage(f"DEBUG generate_physical_geometry for {log_name} - rec_thr_point type: {type(rec_thr_point)}, value: {rec_thr_point}", PLUGIN_TAG, Qgis.Info)
       # --- End log points ---
       ...
       if runway_width is not None and runway_width > 0:
           try:
               runway_geom = self._create_runway_aligned_rectangle(thr_point, rec_thr_point, 0.0, runway_width / 2.0, f"Runway Pavement {log_name}")
               if runway_geom: attributes = {'RWY_Name': runway_name, 'Type': 'Runway Pavement', 'Width_m': runway_width, 'Length_m': round(params['length'], 3), 'MOS_Ref': 'MOS 139 6.2.3'}; generated_elements.append(('Runway', runway_geom, attributes))
               else: QgsMessageLog.logMessage(f"Failed generate runway pavement geometry for {log_name}.", PLUGIN_TAG, level=Qgis.Warning)
           except Exception as e: QgsMessageLog.logMessage(f"Error calc Runway Pavement {log_name}: {e}", PLUGIN_TAG, level=Qgis.Warning)
       else: QgsMessageLog.logMessage(f"Skipping Runway Pavement {log_name}: Width not specified.", PLUGIN_TAG, level=Qgis.Info)

       # --- 2. Runway Strips ---
       strip_dims = self._calculate_strip_dimensions(runway_data)
       strip_end_center_p = None; strip_end_center_r = None; strip_length = None
       if strip_dims and all(strip_dims.get(dim) is not None for dim in ['overall_width', 'graded_width', 'extension_length']):
           try:
               ...
               strip_end_center_p = thr_point.project(extension, params['azimuth_r_p'])
               strip_end_center_r = rec_thr_point.project(extension, params['azimuth_p_r'])
               if strip_end_center_p and strip_end_center_r:
                   strip_length = strip_end_center_p.distance(strip_end_center_r)
                   # 2a. Graded Strip
                   graded_strip_geom = self._create_runway_aligned_rectangle(strip_end_center_p, strip_end_center_r, 0.0, graded_width / 2.0, f"Graded Strip {log_name}")
                   ...
                   # 2b. Overall Strip
                   overall_strip_geom = self._create_runway_aligned_rectangle(strip_end_center_p, strip_end_center_r, 0.0, overall_width / 2.0, f"Overall Strip {log_name}")
               ...
               else: QgsMessageLog.logMessage(f"Skipping Strips {log_name}: Invalid strip end points.", PLUGIN_TAG, level=Qgis.Warning); strip_dims = None
           except Exception as e: QgsMessageLog.logMessage(f"Error calculating Strips {log_name}: {e}", PLUGIN_TAG, level=Qgis.Warning); strip_dims = None
       else: QgsMessageLog.logMessage(f"Skipping Strips {log_name}: Strip dimensions incomplete. Dims: {strip_dims}", PLUGIN_TAG, level=Qgis.Info)
       ...
       if resa_dims and resa_dims.get('required') and strip_dims and strip_end_center_p and strip_end_center_r and all(resa_dims.get(k) is not None for k in ['width', 'length']):
           try:
               ...
               # RESA 1 (Primary End)
               resa1_geom = self._create_offset_rectangle(strip_end_center_p, params['azimuth_r_p'], 0.0, resa_length, resa_half_width, f"RESA {primary_desig}")
               if resa1_geom: resa1_attrs = resa_base_attrs.copy(); resa1_attrs['End_Desig'] = primary_desig; generated_elements.append(('RESA', resa1_geom, resa1_attrs))
               ...
               resa2_geom = self._create_offset_rectangle(strip_end_center_r, params['azimuth_p_r'], 0.0, resa_length, resa_half_width, f"RESA {reciprocal_desig}")
               if resa2_geom: resa2_attrs = resa_base_attrs.copy(); resa2_attrs['End_Desig'] = reciprocal_desig; generated_elements.append(('RESA', resa2_geom, resa2_attrs))
           ...
           except Exception as e_resa: QgsMessageLog.logMessage(f"Error calculating RESAs {log_name}: {e_resa}", PLUGIN_TAG, level=Qgis.Warning)
       elif resa_dims and resa_dims.get('required'): QgsMessageLog.logMessage(f"Skipping RESAs {log_name}: Required but strip/RESA dims incomplete.", PLUGIN_TAG, level=Qgis.Info)
       ...
       if shoulder_width is not None and shoulder_width > 0 and runway_width is not None and runway_width > 0:
           try:
               shoulder_total_half_width = (runway_width / 2.0) + shoulder_width
               shoulder_outer_geom = self._create_runway_aligned_rectangle(thr_point, rec_thr_point, 0.0, shoulder_total_half_width, f"Shoulder Outer {log_name}")
               runway_pavement_geom = next((g for type_key, g, attrs in generated_elements if type_key == 'Runway'), None)
               if shoulder_outer_geom and runway_pavement_geom:
                   shoulder_geom = shoulder_outer_geom.difference(runway_pavement_geom)
                   shoulder_geom = shoulder_geom.makeValid() if shoulder_geom and not shoulder_geom.isGeosValid() else shoulder_geom
                   if shoulder_geom and not shoulder_geom.isEmpty():
                       ...
                       generated_elements.append(('Shoulder', shoulder_geom, shoulder_attrs))
                   else: QgsMessageLog.logMessage(f"Shoulder geometry calculation resulted in empty/invalid for {log_name}.", PLUGIN_TAG, level=Qgis.Warning)
               else: QgsMessageLog.logMessage(f"Skipping Shoulder {log_name}: Outer or pavement geom missing.", PLUGIN_TAG, level=Qgis.Warning)
           except Exception as e_shld: QgsMessageLog.logMessage(f"Error calculating Shoulders for {log_name}: {e_shld}", PLUGIN_TAG, level=Qgis.Warning)

       # --- 5. Stopways ---
       # (Add stopway calculation logic here if needed)

       QgsMessageLog.logMessage(f"Physical geometry generated {len(generated_elements)} elements for {log_name}.", PLUGIN_TAG, level=Qgis.Info)
