...
class SafeguardingBuilder:
    ...
    def run_safeguarding_processing(self):
        ...
        if processed_runway_data_list and any_runway_base_data_ok:
            ...
            if physical_geom_group:
                # Define common fields, adding End_Desig for pre-threshold
                ...
                marking_fields = [
                          QgsField("RWY_Name", QVariant.String, self.tr("Runway Name"), 30),
                          QgsField("Type", QVariant.String, self.tr("Element Type"), 50),
                          QgsField("Length_m", QVariant.Double, self.tr("Length (m)"), 12, 3),
                          QgsField("End_Desig", QVariant.String, self.tr("End Designator"), 10),
                          QgsField("MOS_Ref", QVariant.String, self.tr("MOS Reference"), 250)
                        ]
                ...
                for element_type, definition in layer_definitions.items():
                    geom_type_str = definition.get('geom_type', 'Polygon') # Gets 'LineString' or 'Polygon'
                    ...
                    QgsMessageLog.logMessage(f"DEBUG: Creating layer: Type='{element_type}', Name='{layer_display_name}', Geom='{geom_type_str}'", PLUGIN_TAG, level=Qgis.Info)
            
                    # <<< WORKAROUND #4: Use Explicit WKB Type Names >>>
                    layer_uri = ""
                    if geom_type_str == 'LineString':
                      # Use the specific WKB type name recognised by QGIS/OGR
                      layer_uri = f"LineString?crs={target_crs.authid()}&index=yes"
                    elif geom_type_str == 'Polygon':
                      layer_uri = f"Polygon?crs={target_crs.authid()}&index=yes"
                    else:
                      # Fallback or handle other types if needed
                      QgsMessageLog.logMessage(f"Warning: Unsupported geometry type '{geom_type_str}' for layer URI.", PLUGIN_TAG, level=Qgis.Warning)
                      continue # Skip layer creation if type is unknown
            
                    QgsMessageLog.logMessage(f"DEBUG: Using Layer URI: '{layer_uri}'", PLUGIN_TAG, level=Qgis.Info)
                    layer = QgsVectorLayer(layer_uri, layer_name_internal, "memory")
                    # <<< END WORKAROUND #4 >>>
            
                    QgsMessageLog.logMessage(f"DEBUG: Layer object created for '{element_type}'. IsValid={layer.isValid()}", PLUGIN_TAG, level=Qgis.Info)
                    
                    if layer.isValid() and layer.dataProvider() and layer.dataProvider().addAttributes(definition['fields']):
                      # <<< LOG: Attributes added OK >>>
                      QgsMessageLog.logMessage(f"DEBUG: Attributes added for '{element_type}'.", PLUGIN_TAG, level=Qgis.Info)
                      layer.updateFields()
                      layer.setName(layer_display_name)
                      layer.setCustomProperty("safeguarding_style_key", style_key_map.get(element_type)) # Use get for safety
                      
                      # <<< LOG: Before startEditing >>>
                      QgsMessageLog.logMessage(f"DEBUG: Attempting startEditing for '{element_type}'.", PLUGIN_TAG, level=Qgis.Info)
                      if layer.startEditing():
                        # <<< LOG: startEditing OK >>>
                        QgsMessageLog.logMessage(f"DEBUG: startEditing successful for '{element_type}'. Adding to physical_layers dict.", PLUGIN_TAG, level=Qgis.Info)
                        physical_layers[element_type] = layer;
                      else:
                        # Failed startEditing
                        QgsMessageLog.logMessage(f"Failed startEditing physical layer {layer_display_name}", PLUGIN_TAG, level=Qgis.Warning)
                        physical_layers[element_type] = None # Explicitly mark as None if editing fails
                    else:
                      # Layer invalid, provider issue, or attribute add failed
                      prov_ok = layer.isValid() and layer.dataProvider()
                      attr_ok = prov_ok and layer.dataProvider().addAttributes(definition['fields']) # Recheck condition
                      QgsMessageLog.logMessage(f"Failed init physical layer {layer_display_name} (LayerValid={layer.isValid()}, ProvOK={prov_ok}, AttrAddOK={attr_ok})", PLUGIN_TAG, level=Qgis.Critical)
                      physical_layers[element_type] = None # Explicitly mark as None if init fails
                      
                # <<< LOG: After Layer Creation Loop >>>
                QgsMessageLog.logMessage(f"DEBUG: Finished creating physical layer structures. Layers created: {list(physical_layers.keys())}", PLUGIN_TAG, level=Qgis.Info)

                # Populate layers
                physical_geom_features_added = {element_type: False for element_type in physical_layers.keys()}
                # <<< Logging Change: Changed Log Level >>>
                QgsMessageLog.logMessage("Populating physical geometry layers...", PLUGIN_TAG, level=Qgis.Info); QgsMessageLog.logMessage("", PLUGIN_TAG, Qgis.Info) # Flush
                for rwy_data in processed_runway_data_list:
                  runway_name_log = rwy_data.get('short_name', f"RWY_{rwy_data.get('original_index','?')}")
                  QgsMessageLog.logMessage(f"DEBUG: Processing physical elements for {runway_name_log}", PLUGIN_TAG, level=Qgis.Info); QgsMessageLog.logMessage("", PLUGIN_TAG, Qgis.Info) # Flush
                  try:
                    ...
                    # <<< Logging Change: Removed redundant strip calculation call here >>>
                    # strip_dims = self._calculate_strip_dimensions(rwy_data) # This recalculates, use stored if available
                    # strip_dims = rwy_data.get('calculated_strip_dims') # Get potentially stored dims
                    # if strip_dims:
                    #   QgsMessageLog.logMessage(f"DEBUG: Using stored strip_dims for {runway_name_log}: {strip_dims!r}", PLUGIN_TAG, level=Qgis.Info); QgsMessageLog.logMessage("", PLUGIN_TAG, Qgis.Info) # Flush
                    # Note: If strip_dims are needed later (e.g., for OLS) ensure they are present in rwy_data
                      
                    if not generated_elements:
                      QgsMessageLog.logMessage(f"DEBUG: Got NO elements back from generate_physical_geometry for {runway_name_log}", PLUGIN_TAG, level=Qgis.Info); QgsMessageLog.logMessage("", PLUGIN_TAG, Qgis.Info) # Flush
                      continue # Skip if nothing generated
                    else:
                      QgsMessageLog.logMessage(f"DEBUG: Got {len(generated_elements)} elements back from generate_physical_geometry for {runway_name_log}", PLUGIN_TAG, level=Qgis.Info); QgsMessageLog.logMessage("", PLUGIN_TAG, Qgis.Info) # Flush
                      
                      
                    # <<< DETAILED FEATURE ADDITION LOGGING >>>
                    for i, (element_type, geometry, attributes) in enumerate(generated_elements):
                      target_layer = physical_layers.get(element_type)
                      # Log 5 (Check target layer and geom)
                      QgsMessageLog.logMessage(f"DEBUG: Processing Element {i+1}/{len(generated_elements)}: Type='{element_type}'", PLUGIN_TAG, level=Qgis.Info); QgsMessageLog.logMessage("", PLUGIN_TAG, Qgis.Info) # Flush
                      is_target_layer_valid = target_layer is not None and target_layer.isValid()
                      is_target_layer_editable = is_target_layer_valid and target_layer.isEditable()
                      is_geom_valid_initial = geometry is not None and not geometry.isEmpty()
                      QgsMessageLog.logMessage(f"DEBUG:   TargetLayerValid={is_target_layer_valid}, Editable={is_target_layer_editable}, GeomValidInitial={is_geom_valid_initial}", PLUGIN_TAG, level=Qgis.Info); QgsMessageLog.logMessage("", PLUGIN_TAG, Qgis.Info) # Flush
                      
                      if is_target_layer_valid and is_target_layer_editable and is_geom_valid_initial:
                        # Check/Fix Geometry Validity
                        is_geom_geos_valid = geometry.isGeosValid()
                        QgsMessageLog.logMessage(f"DEBUG:   Geom IsGeosValid={is_geom_geos_valid}", PLUGIN_TAG, level=Qgis.Info); QgsMessageLog.logMessage("", PLUGIN_TAG, Qgis.Info) # Flush
                        if not is_geom_geos_valid:
                          QgsMessageLog.logMessage(f"DEBUG:   Attempting makeValid() for {element_type}", PLUGIN_TAG, level=Qgis.Info); QgsMessageLog.logMessage("", PLUGIN_TAG, Qgis.Info) # Flush
                          geometry = geometry.makeValid()
                          is_geom_geos_valid = geometry is not None and not geometry.isEmpty() and geometry.isGeosValid()
                          QgsMessageLog.logMessage(f"DEBUG:   Result of makeValid(): GeomValid={is_geom_geos_valid}", PLUGIN_TAG, level=Qgis.Info); QgsMessageLog.logMessage("", PLUGIN_TAG, Qgis.Info) # Flush
                          
                        if geometry and not geometry.isEmpty() and is_geom_geos_valid:
                          ...
                          # <<< ADD THIS LOG >>>
                          try:
                            geom_wkt = geometry.asWkt() if geometry else "None"
                            feature_attrs = feature.attributes() # Get attributes actually set on the feature
                            QgsMessageLog.logMessage(f"DEBUG:   Feature Details: GeomWKT='{geom_wkt}', Attrs={feature_attrs}", PLUGIN_TAG, level=Qgis.Info); QgsMessageLog.logMessage("", PLUGIN_TAG, Qgis.Info) # Flush
                          except Exception as e_debug:
                            QgsMessageLog.logMessage(f"DEBUG: Error getting feature details: {e_debug}", PLUGIN_TAG, level=Qgis.Warning)
                          # <<< END ADDED LOG >>>  
                          
                          # <<< ADD GEOMETRY LENGTH CHECK FOR LINES >>>
                          is_line = (element_type == 'DisplacedThresholdMarking' or element_type == 'PreThresholdAreaMarking')
                          if is_line:
                            try:
                              geom_length = geometry.length()
                              QgsMessageLog.logMessage(f"DEBUG:   Line Geometry Length for {element_type}: {geom_length}", PLUGIN_TAG, level=Qgis.Info); QgsMessageLog.logMessage("", PLUGIN_TAG, Qgis.Info) # Flush
                              if geom_length is None or geom_length < 1e-9: # Check for zero or near-zero length
                                QgsMessageLog.logMessage(f"WARNING:   Line geometry length is zero or near-zero for {element_type}. Skipping addFeature.", PLUGIN_TAG, level=Qgis.Warning); QgsMessageLog.logMessage("", PLUGIN_TAG, Qgis.Warning) # Flush
                                continue # Skip to the next element if line length is zero
                            except Exception as e_len:
                              QgsMessageLog.logMessage(f"WARNING: Error getting line geometry length for {element_type}: {e_len}", PLUGIN_TAG, level=Qgis.Warning); QgsMessageLog.logMessage("", PLUGIN_TAG, Qgis.Warning) # Flush
                              # Decide whether to continue or skip based on error
                              # continue # Optional: Skip if length check fails
                          
                          # <<< TRY WORKAROUND #2: Explicit ID using -1 >>>
                          try:
                            # <<< Use -1 for invalid/null ID >>>
                            feature.setId(-1)
                            QgsMessageLog.logMessage(f"DEBUG:   Set feature ID to -1 for {element_type}", PLUGIN_TAG, level=Qgis.Info); QgsMessageLog.logMessage("", PLUGIN_TAG, Qgis.Info) # Flush
                          except Exception as e_id:
                            # Log if setting ID fails for some reason
                            QgsMessageLog.logMessage(f"DEBUG:   Failed to set -1 ID for {element_type}: {e_id}", PLUGIN_TAG, level=Qgis.Warning); QgsMessageLog.logMessage("", PLUGIN_TAG, Qgis.Warning) # Flush
                          # <<< END WORKAROUND #2 >>>
                          
                          # Log 6 (Before addFeature)
                          QgsMessageLog.logMessage(f"DEBUG:   Attempting addFeature for {element_type} to layer {target_layer.name()}", PLUGIN_TAG, level=Qgis.Info); QgsMessageLog.logMessage("", PLUGIN_TAG, Qgis.Info) # Flush
                          add_ok = target_layer.dataProvider().addFeature(feature)
                          
                          # Log 7 (After addFeature)
                          QgsMessageLog.logMessage(f"DEBUG:   addFeature result for {element_type}: {add_ok}", PLUGIN_TAG, level=Qgis.Info); QgsMessageLog.logMessage("", PLUGIN_TAG, Qgis.Info) # Flush
                          
                          if add_ok:
                            physical_geom_features_added[element_type] = True
                            any_physical_geom_ok = True
                          else:
                            # Log specific error but continue loop for this runway
                            QgsMessageLog.logMessage(f"WARNING    ERROR adding physical feature {element_type} for {runway_name_log}", PLUGIN_TAG, level=Qgis.Warning); QgsMessageLog.logMessage("", PLUGIN_TAG, Qgis.Warning) # Flush
                        else:
                          QgsMessageLog.logMessage(f"DEBUG:   Skipping add feature for {element_type} due to invalid geometry after potential makeValid()", PLUGIN_TAG, level=Qgis.Warning); QgsMessageLog.logMessage("", PLUGIN_TAG, Qgis.Warning) # Flush
                      else:
                        # Log why adding was skipped
                        skip_reason = []
                        if not is_target_layer_valid: skip_reason.append("Target layer invalid/missing")
                        elif not is_target_layer_editable: skip_reason.append("Target layer not editable")
                        if not is_geom_valid_initial: skip_reason.append("Initial geometry invalid/empty")
                        QgsMessageLog.logMessage(f"DEBUG:   Skipping creation/add for {element_type} because: {', '.join(skip_reason)}", PLUGIN_TAG, level=Qgis.Warning); QgsMessageLog.logMessage("", PLUGIN_TAG, Qgis.Warning) # Flush
                    # <<< END DETAILED FEATURE ADDITION LOGGING >>>
                        
                  except Exception as e_phys:
                    # Catch errors during processing a specific runway's physical geometry
                    QgsMessageLog.logMessage(f"CRITICAL Error during feature population loop for {runway_name_log}: {e_phys}", PLUGIN_TAG, level=Qgis.Critical)
                    QgsMessageLog.logMessage(f"Traceback:\n{traceback.format_exc()}", PLUGIN_TAG, level=Qgis.Critical)
                    # Continue to next runway to allow partial success
                    continue
                ...
                for element_type, layer in physical_layers.items():
                  # Check if layer object actually exists and is valid
                  if layer and layer.isValid():
                    # Only proceed if the layer is editable (startEditing succeeded)
                    if layer.isEditable():
                      if physical_geom_features_added.get(element_type, False):
                        QgsMessageLog.logMessage(f"DEBUG: Attempting commitChanges for '{layer.name()}' ({element_type})", PLUGIN_TAG, level=Qgis.Info) # Log before commit
                        if layer.commitChanges():
                          QgsMessageLog.logMessage(f"DEBUG: Commit successful for '{layer.name()}'. Adding to map.", PLUGIN_TAG, level=Qgis.Info) # Log after commit
                          ...
                          if node:
                            ...
                            QgsMessageLog.logMessage(f"DEBUG: Layer '{layer.name()}' added to group.", PLUGIN_TAG, level=Qgis.Info) # Log node add success
                          else:
                            QgsMessageLog.logMessage(f"Warn: Failed add node for committed layer '{layer.name()}'.", PLUGIN_TAG, level=Qgis.Warning)
                        else:
                          commit_errors = True
                          QgsMessageLog.logMessage(f"Commit FAILED for '{layer.name()}': {layer.commitErrors()}", PLUGIN_TAG, level=Qgis.Warning)
                          # Rolling back changes for this layer
                          layer.rollBack()
                          QgsMessageLog.logMessage(f"Rolled back changes for '{layer.name()}'.", PLUGIN_TAG, level=Qgis.Info)
                      else:
                        # No features were successfully added, roll back to remove editing state
                        QgsMessageLog.logMessage(f"DEBUG: No features added for '{layer.name()}' ({element_type}). Rolling back.", PLUGIN_TAG, level=Qgis.Info)
                        layer.rollBack()
                    else:
                      # Layer wasn't editable (startEditing likely failed)
                      QgsMessageLog.logMessage(f"DEBUG: Skipping commit/rollback for '{layer.name()}' ({element_type}) as it was not editable.", PLUGIN_TAG, level=Qgis.Info)
                  else:
                    # Layer object was None or invalid (layer creation likely failed)
                    QgsMessageLog.logMessage(f"DEBUG: Skipping commit for element type '{element_type}' as layer object is invalid or None.", PLUGIN_TAG, level=Qgis.Info)
                    
                if commit_errors:
                  self.iface.messageBar().pushMessage(self.tr("Warning"), self.tr("Commit errors occurred on physical layers."), level=Qgis.Warning, duration=7)
                  
            # This else corresponds to 'if physical_geom_group:'
            else:
    ...
    def generate_physical_geometry(self, runway_data: dict) -> Optional[List[Tuple[str, QgsGeometry, dict]]]:
      ...
      disp_thr_1 = runway_data.get('thr_displaced_1', 0.0) # Default to 0 if missing
      disp_thr_2 = runway_data.get('thr_displaced_2', 0.0) # Default to 0 if missing
      # Basic validation (should already be float or None from input processing)
      disp_thr_1 = float(disp_thr_1) if isinstance(disp_thr_1, (int, float)) else 0.0
      disp_thr_2 = float(disp_thr_2) if isinstance(disp_thr_2, (int, float)) else 0.0
      
      pre_area_len_1 = runway_data.get('thr_pre_area_1') # Raw value
      pre_area_len_2 = runway_data.get('thr_pre_area_2') # Raw value
      try:
        pre_area_len_1 = float(pre_area_len_1) if pre_area_len_1 is not None else 0.0
      except (ValueError, TypeError):
        QgsMessageLog.logMessage(f"Invalid Pre-Threshold Area 1 length '{pre_area_len_1}' for {log_name}, using 0.0.", PLUGIN_TAG, level=Qgis.Warning)
        pre_area_len_1 = 0.0
      try:
        pre_area_len_2 = float(pre_area_len_2) if pre_area_len_2 is not None else 0.0
      except (ValueError, TypeError):
        QgsMessageLog.logMessage(f"Invalid Pre-Threshold Area 2 length '{pre_area_len_2}' for {log_name}, using 0.0.", PLUGIN_TAG, level=Qgis.Warning)
        pre_area_len_2 = 0.0
      ...
      if rwy_params is None:
        QgsMessageLog.logMessage(f"Skipping physical geom {log_name}: Failed to get base runway parameters.", PLUGIN_TAG, level=Qgis.Warning)
        return None
      
      # --- Get Physical Runway Endpoints & Length --- <<< NEW >>>
      ...
      if physical_endpoints_result is None:
          QgsMessageLog.logMessage(f"Skipping physical geom {log_name}: Failed to calculate physical endpoints.", PLUGIN_TAG, level=Qgis.Warning)
          return None
      phys_p_start, phys_p_end, physical_length = physical_endpoints_result
      # <<< END NEW >>>
      
      generated_elements: List[Tuple[str, QgsGeometry, dict]] = []
      pavement_corners_landing = {} # Corners of the landing area pavement
      ...
      if runway_width is not None and runway_width > 0:
        try:
          ...
          # Corners based on LANDING thresholds
          ...
          if all([thr_l, thr_r, rec_l, rec_r]):
            pavement_corners_landing = {'thr_l': thr_l, 'thr_r': thr_r, 'rec_l': rec_l, 'rec_r': rec_r} # Store landing corners
            ...
            if landing_pavement_geom:
              # Use the length between thresholds for this feature's attribute
              landing_length = rwy_params['length']
              physical_refs = ols_dimensions.get_physical_refs() # Get refs
            ...
            else: QgsMessageLog.logMessage(f"Failed to generate landing pavement geometry for {log_name}.", PLUGIN_TAG, level=Qgis.Warning); pavement_corners_landing = {}
          else: QgsMessageLog.logMessage(f"Failed to calculate landing pavement corners for {log_name}.", PLUGIN_TAG, level=Qgis.Warning); pavement_corners_landing = {}
        except Exception as e: QgsMessageLog.logMessage(f"Error calculating Landing Pavement for {log_name}: {e}", PLUGIN_TAG, level=Qgis.Warning); pavement_corners_landing = {}
      else: QgsMessageLog.logMessage(f"Skipping Landing Pavement for {log_name}: Width not specified.", PLUGIN_TAG, level=Qgis.Info)
      ...
      if runway_width is not None and runway_width > 0:
        ...
        if disp_thr_1 > 1e-6:
            try:
                # Start point is physical end, end point is landing threshold
                start_point = phys_p_start
                end_point = thr_point
                # Use _create_runway_aligned_rectangle (or similar logic) between physical end and threshold
                # We need corners relative to these points
                ...
                p_end_l = end_point.project(half_width, rwy_params['azimuth_perp_l']) # = thr_l if calculated
                p_end_r = end_point.project(half_width, rwy_params['azimuth_perp_r']) # = thr_r if calculated
                if all([p_start_l, p_start_r, p_end_l, p_end_r]):
                    geom = self._create_polygon_from_corners([p_start_l, p_start_r, p_end_r, p_end_l], f"Pre-Threshold {log_name} Primary")
                    if geom:
                        # Reference could be the same as pavement, or maybe specific to displacement markings
                        physical_refs = ols_dimensions.get_physical_refs()
                        pavement_ref = physical_refs.get('pavement', "MOS 139 6.2.3") # Re-use pavement ref for now
                        ...
                        # Use a distinct element key, e.g., 'PreThresholdRunway'
                        pre_threshold_features.append(('PreThresholdRunway', geom, attributes))
                    else: QgsMessageLog.logMessage(f"Failed generate Pre-Threshold geom for Primary {log_name}.", PLUGIN_TAG, level=Qgis.Warning)
                else: QgsMessageLog.logMessage(f"Failed calculate corners for Pre-Threshold Primary {log_name}.", PLUGIN_TAG, level=Qgis.Warning)
            except Exception as e: QgsMessageLog.logMessage(f"Error generating Pre-Threshold Primary {log_name}: {e}", PLUGIN_TAG, level=Qgis.Warning)
          
        # Reciprocal End Displacement
        if disp_thr_2 > 1e-6:
            try:
                # Start point is physical end, end point is landing threshold
                ...
                r_end_l = end_point.project(half_width, rwy_params['azimuth_perp_l']) # = rec_l if calculated
                r_end_r = end_point.project(half_width, rwy_params['azimuth_perp_r']) # = rec_r if calculated
                if all([r_start_l, r_start_r, r_end_l, r_end_r]):
                    geom = self._create_polygon_from_corners([r_start_l, r_start_r, r_end_r, r_end_l], f"Pre-Threshold {log_name} Reciprocal")
                    if geom:
                        physical_refs = ols_dimensions.get_physical_refs()
                        pavement_ref = physical_refs.get('pavement', "MOS 139 6.2.3")
                        ...
                        pre_threshold_features.append(('PreThresholdRunway', geom, attributes))
                    else: QgsMessageLog.logMessage(f"Failed generate Pre-Threshold geom for Reciprocal {log_name}.", PLUGIN_TAG, level=Qgis.Warning)
                else: QgsMessageLog.logMessage(f"Failed calculate corners for Pre-Threshold Reciprocal {log_name}.", PLUGIN_TAG, level=Qgis.Warning)
            except Exception as e: QgsMessageLog.logMessage(f"Error generating Pre-Threshold Reciprocal {log_name}: {e}", PLUGIN_TAG, level=Qgis.Warning)
          
        generated_elements.extend(pre_threshold_features) # Add generated features to the main list
        ...
        if runway_width is not None and runway_width > 0:
            ...
            if pre_area_len_1 > 1e-6: # Check if length is significant
                  try:
                      # Start point for the area is the physical end of the pavement
                      area_start_point = phys_p_start
                      # Direction is "outwards" from the physical end (same as projecting backwards from THR)
                      outward_azimuth = rwy_params['azimuth_r_p']
                    
                      geom = self._create_rectangle_from_start(
                          area_start_point, outward_azimuth, pre_area_len_1,
                          half_width, f"Pre-Threshold Area {log_name} Primary"
                      )
                      if geom:
                          # Use a generic reference or leave blank?
                      ...
                      else: QgsMessageLog.logMessage(f"Failed generate Pre-Threshold Area geom for Primary {log_name}.", PLUGIN_TAG, level=Qgis.Warning)
                  except Exception as e: QgsMessageLog.logMessage(f"Error generating Pre-Threshold Area Primary {log_name}: {e}", PLUGIN_TAG, level=Qgis.Warning)
          
            # Area beyond Reciprocal Physical End
            if pre_area_len_2 > 1e-6: # Check if length is significant
                  try:
                      # Start point for the area is the physical end of the pavement
                      ...
                      geom = self._create_rectangle_from_start(
                          area_start_point, outward_azimuth, pre_area_len_2,
                          half_width, f"Pre-Threshold Area {log_name} Reciprocal"
                      )
                      if geom:
                          ...
                          pre_threshold_area_features.append(('PreThresholdArea', geom, attributes))
                      else: QgsMessageLog.logMessage(f"Failed generate Pre-Threshold Area geom for Reciprocal {log_name}.", PLUGIN_TAG, level=Qgis.Warning)
                  except Exception as e: QgsMessageLog.logMessage(f"Error generating Pre-Threshold Area Reciprocal {log_name}: {e}", PLUGIN_TAG, level=Qgis.Warning)
          
            generated_elements.extend(pre_threshold_area_features) # Add generated features
        ...
        if disp_thr_1 > 1e-6:
          try:
              # Line FROM physical start TO landing threshold
              ...
              else: QgsMessageLog.logMessage(f"Failed generate geometry for Primary Displaced Marking {log_name}.", PLUGIN_TAG, level=Qgis.Warning)
          except Exception as e: QgsMessageLog.logMessage(f"Error generating Primary Displaced Marking {log_name}: {e}", PLUGIN_TAG, level=Qgis.Warning)
          
        # Marking for Reciprocal Displaced Threshold
        if disp_thr_2 > 1e-6:
          try:
              # Line FROM physical end TO landing threshold
              line_geom = QgsGeometry.fromPolylineXY([phys_p_end, rec_thr_point])
              if line_geom and not line_geom.isEmpty():
                line_len = line_geom.length()
                attributes = {'RWY_Name': runway_name, 'Type': 'Displaced Threshold Marking', 'End_Desig': primary_desig, 'Length_m': round(line_len, 3) if line_len else None, 'MOS_Ref': marking_ref}
                displaced_marking_features.append(('DisplacedThresholdMarking', line_geom, attributes))
              else: QgsMessageLog.logMessage(f"Failed generate geometry for Reciprocal Displaced Marking {log_name}.", PLUGIN_TAG, level=Qgis.Warning)
          except Exception as e: QgsMessageLog.logMessage(f"Error generating Reciprocal Displaced Marking {log_name}: {e}", PLUGIN_TAG, level=Qgis.Warning)
          
        generated_elements.extend(displaced_marking_features)
        QgsMessageLog.logMessage(f"DEBUG: Added {len(displaced_marking_features)} DisplacedThresholdMarking features for {log_name}", PLUGIN_TAG, level=Qgis.Info)
        ...
        if pre_area_len_1 > 1e-6:
            try:
              ...
              # Line FROM outermost point TO physical start point
              ...
              else: QgsMessageLog.logMessage(f"Failed generate geometry for Primary Pre-Area Marking {log_name}.", PLUGIN_TAG, level=Qgis.Warning)
            except Exception as e: QgsMessageLog.logMessage(f"Error generating Primary Pre-Area Marking {log_name}: {e}", PLUGIN_TAG, level=Qgis.Warning)
          
        # Marking for Reciprocal Pre-Threshold Area
        if pre_area_len_2 > 1e-6:
            try:
              ...
              # Line FROM outermost point TO physical end point
              line_geom = QgsGeometry.fromPolylineXY([outermost_r, phys_p_end])
              if line_geom and not line_geom.isEmpty():
                line_len = line_geom.length()
                attributes = {'RWY_Name': runway_name, 'Type': 'Pre-Threshold Area Marking', 'End_Desig': primary_desig, 'Length_m': round(line_len, 3) if line_len else None, 'MOS_Ref': marking_ref}
                pre_area_marking_features.append(('PreThresholdAreaMarking', line_geom, attributes))
              else: QgsMessageLog.logMessage(f"Failed generate geometry for Reciprocal Pre-Area Marking {log_name}.", PLUGIN_TAG, level=Qgis.Warning)
            except Exception as e: QgsMessageLog.logMessage(f"Error generating Reciprocal Pre-Area Marking {log_name}: {e}", PLUGIN_TAG, level=Qgis.Warning)
          
        generated_elements.extend(pre_area_marking_features)
        QgsMessageLog.logMessage(f"DEBUG: Added {len(pre_area_marking_features)} PreThresholdAreaMarking features for {log_name}", PLUGIN_TAG, level=Qgis.Info)
        
      # --- 2. Runway Shoulders ---
      if shoulder_width is not None and shoulder_width > 0 and runway_width is not None and runway_width > 0:
          try:
              # Calculate pavement corners based on PHYSICAL endpoints
              half_width = runway_width / 2.0
              ...
              # Calculate the 4 outer corner points by projecting from physical pavement corners
              ...
              # Use PHYSICAL length for shoulder attributes
              ...
              # Create Left Shoulder Polygon (using physical corners)
              if all([outer_start_l, outer_end_l]):
                  ...
                  else: QgsMessageLog.logMessage(f"Failed generate left shoulder geometry for {log_name}.", PLUGIN_TAG, level=Qgis.Warning)
              else: QgsMessageLog.logMessage(f"Failed calculate outer corners for left shoulder for {log_name}.", PLUGIN_TAG, level=Qgis.Warning)
            
              # Create Right Shoulder Polygon (using physical corners)
              if all([outer_start_r, outer_end_r]):
                  right_corners = [phys_start_r, phys_end_r, outer_end_r, outer_start_r] # Adjusted winding order
                  ...
                  else: QgsMessageLog.logMessage(f"Failed generate right shoulder geometry for {log_name}.", PLUGIN_TAG, level=Qgis.Warning)
              else: QgsMessageLog.logMessage(f"Failed calculate outer corners for right shoulder for {log_name}.", PLUGIN_TAG, level=Qgis.Warning)
        ...
        QgsMessageLog.logMessage(f"Skipping Shoulders for {log_name}: Runway width missing.", PLUGIN_TAG, level=Qgis.Info)
        
        
      # --- 3. Runway Strips ---
      # <<< MODIFY to use PHYSICAL endpoints >>>
      strip_dims = None; strip_end_center_p = None; strip_end_center_r = None; strip_length = None
      try:
        # Get strip parameter calculation function from ols_dimensions
        # strip_dims = self._calculate_strip_dimensions(runway_data) # OLD
        type1_abbr = ols_dimensions.get_runway_type_abbr(runway_data.get('type1'))
        strip_dims = ols_dimensions.get_strip_params(
            int(runway_data.get('arc_num', 0)), type1_abbr, runway_width
        )
        # <<< FIX: Store calculated dims back into the input dictionary >>>
        # Store the result (which could be None if lookup failed)
        ...
        if strip_dims:
          ...
          QgsMessageLog.logMessage(f"DEBUG (generate_physical_geometry): Failed to calc/store strip_dims for {log_name}", PLUGIN_TAG, level=Qgis.Warning)
        # <<< END FIX >>>
        ...
        if strip_dims and all(strip_dims.get(dim) is not None for dim in ['overall_width', 'graded_width', 'extension_length']):
          ...
          # Calculate strip ends based on PHYSICAL runway ends + extension
          strip_end_center_p = phys_p_start.project(extension, rwy_params['azimuth_r_p']) # Extend "backwards" from physical start
          strip_end_center_r = phys_p_end.project(extension, rwy_params['azimuth_p_r'])   # Extend "forwards" from physical end
          ...
          if strip_end_center_p and strip_end_center_r:
            ...
            # 3a. Graded Strip
            # Use strip end points which are relative to physical ends + extension
            ...
            else: QgsMessageLog.logMessage(f"Failed generate graded strip geometry for {log_name}.", PLUGIN_TAG, level=Qgis.Warning)
            ...
            else: QgsMessageLog.logMessage(f"Failed generate overall strip geometry for {log_name}.", PLUGIN_TAG, level=Qgis.Warning)
          else: strip_dims = None; QgsMessageLog.logMessage(f"Skipping Strips for {log_name}: Invalid strip end points calculation.", PLUGIN_TAG, level=Qgis.Warning)
      ...
      # --- 4. RESAs ---
      # <<< MODIFY to use strip ends derived from PHYSICAL endpoints >>>
      try:
        # Get RESA parameter calculation function from ols_dimensions
        type1_abbr = ols_dimensions.get_runway_type_abbr(runway_data.get('type1'))
        ...
        # Check if RESAs required AND strip ends were successfully calculated earlier
        if resa_dims and resa_dims.get('required') and strip_end_center_p and strip_end_center_r and runway_width is not None and runway_width > 0:
            ...
            # RESA Width is based on runway width (not shoulder) - Use 2.0 * runway_width (calculation logic remains)
            resa_width = 2.0 * runway_width
            ...
            # Start point: strip_end_center_p (calculated relative to physical end)
            # Outward azimuth: azimuth_r_p
            try:
                resa1_geom = self._create_rectangle_from_start(
                    strip_end_center_p, rwy_params['azimuth_r_p'], resa_length, resa_half_width, f"RESA {primary_desig}"
                )
                if resa1_geom:
                ...
                else: QgsMessageLog.logMessage(f"Failed generate RESA {primary_desig} geometry for {log_name}.", PLUGIN_TAG, level=Qgis.Warning)
            ...
            # Start point: strip_end_center_r (calculated relative to physical end)
            # Outward azimuth: azimuth_p_r
            try:
                resa2_geom = self._create_rectangle_from_start(
                    strip_end_center_r, rwy_params['azimuth_p_r'], resa_length, resa_half_width, f"RESA {reciprocal_desig}"
                )
                if resa2_geom:
                ...
                else: QgsMessageLog.logMessage(f"Failed generate RESA {reciprocal_desig} geometry for {log_name}.", PLUGIN_TAG, level=Qgis.Warning)
            ...
            QgsMessageLog.logMessage(f"Skipping RESAs for {log_name}: Required but strip data/runway width incomplete.", PLUGIN_TAG, level=Qgis.Info)
        # else: RESAs not required
      ...
      # --- 5. Stopways ---
      # (Placeholder - Would also likely depend on physical endpoints)
        
      QgsMessageLog.logMessage(f"Physical geometry finished for {log_name}. Generated {len(generated_elements)} element features.", PLUGIN_TAG, level=Qgis.Info)
      
      QgsMessageLog.logMessage(f"DEBUG: Returning {len(generated_elements)} total element features for {log_name}", PLUGIN_TAG, level=Qgis.Info)
    ...
    def _generate_airport_wide_ols(self, processed_runway_data_list: List[dict],
        ...
        # Get IHS base height from centralized parameters
        ihs_base_height_agl = ols_dimensions.get_ihs_base_height() # Assumes this helper exists
        if ihs_base_height_agl is None:
             QgsMessageLog.logMessage("Cannot generate Airport-Wide OLS: Failed to retrieve IHS base height.", PLUGIN_TAG, level=Qgis.Critical)
             return False
        ...
        # --- 1. Generate Individual Strip Outline Geometries (Relative to Physical Ends) ---
        strip_outline_geoms: List[QgsGeometry] = []
        QgsMessageLog.logMessage("Generating individual strip outlines for IHS base (using physical ends)...", PLUGIN_TAG, level=Qgis.Info)
        for rwy_data in processed_runway_data_list:
            ...
            runway_width = rwy_data.get('width') # Needed for strip param calculation
          
            # Get displacement values for this runway
            ...
            try:
                ...
                QgsMessageLog.logMessage(f"Invalid displacement values for {rwy_name} during IHS base calc, skipping.", PLUGIN_TAG, level=Qgis.Warning)
                continue # Skip this runway if displacement invalid
          
            # Basic checks
            if not all([thr_point, rec_thr_point, arc_num_str]):
                QgsMessageLog.logMessage(f"Missing essential data for {rwy_name} during IHS base calc, skipping.", PLUGIN_TAG, level=Qgis.Warning)
                continue
            try:
                 arc_num = int(arc_num_str)
                 highest_arc_num = max(highest_arc_num, arc_num)
            except ValueError:
                 QgsMessageLog.logMessage(f"Invalid ARC number for {rwy_name} during IHS base calc, skipping.", PLUGIN_TAG, level=Qgis.Warning)
                 continue
          
            # Track highest precision type string (using FULL STRINGS)
            ...
            # Get runway parameters (azimuths) based on landing thresholds
            rwy_params = self._get_runway_parameters(thr_point, rec_thr_point)
            if not rwy_params:
                QgsMessageLog.logMessage(f"Failed to get runway params for {rwy_name} during IHS base calc, skipping.", PLUGIN_TAG, level=Qgis.Warning)
                continue
          
            # Get physical endpoints <<< NEW >>>
            physical_endpoints_result = self._get_physical_runway_endpoints(
                thr_point, rec_thr_point, disp_thr_1, disp_thr_2, rwy_params
            )
            if physical_endpoints_result is None:
                QgsMessageLog.logMessage(f"Failed to get physical endpoints for {rwy_name} during IHS base calc, skipping.", PLUGIN_TAG, level=Qgis.Warning)
                continue
            phys_p_start, phys_p_end, _ = physical_endpoints_result # Get physical ends
          
            # Get strip dimensions (especially extension length)
            # Check if already calculated and stored, otherwise calculate now
            strip_dims = rwy_data.get('calculated_strip_dims')
            if not strip_dims:
                 # Need type abbreviation for lookup
                 current_max_type_abbr = ols_dimensions.get_runway_type_abbr(current_max_type_str)
                 strip_dims = ols_dimensions.get_strip_params(arc_num, current_max_type_abbr, runway_width)
                 if not strip_dims:
                      QgsMessageLog.logMessage(f"Failed to get strip dimensions for {rwy_name} during IHS base calc, skipping.", PLUGIN_TAG, level=Qgis.Warning)
                      continue
              
            strip_width = strip_dims.get('overall_width') # Use overall width for IHS boundary
            strip_ext = strip_dims.get('extension_length')
            if strip_width is None or strip_width <= 0 or strip_ext is None:
                QgsMessageLog.logMessage(f"Invalid strip width/extension for {rwy_name} during IHS base calc, skipping.", PLUGIN_TAG, level=Qgis.Warning)
                continue
          
            # Calculate strip end center points based on PHYSICAL ends + extension <<< MODIFIED >>>
            strip_end_p = phys_p_start.project(strip_ext, rwy_params['azimuth_r_p']) # Extend "backwards" from physical start
            strip_end_r = phys_p_end.project(strip_ext, rwy_params['azimuth_p_r'])   # Extend "forwards" from physical end
            if not strip_end_p or not strip_end_r:
                QgsMessageLog.logMessage(f"Failed to calculate strip end points for {rwy_name} during IHS base calc, skipping.", PLUGIN_TAG, level=Qgis.Warning)
                continue
          
            # Get IHS end radius parameter (used for rounding strip ends)
            # Use the full descriptive string for the CURRENT runway's max type
            ...
            # Fallback if radius not defined or invalid - use half overall strip width
            if ihs_end_radius is None or not isinstance(ihs_end_radius, (int, float)) or ihs_end_radius <= 0:
                 ihs_end_radius = strip_width / 2.0
                 if ihs_params: # Add note to ref if fallback used
                      ihs_params['ref'] = ihs_params.get('ref', '') + " (Radius Fallback)"
              
            # Generate the strip outline geometry using calculated strip ends
            try:
                 # Buffer the calculated strip ends
                 ...
                 # Connector polygon corners are based on the strip ends and radius
                 ...
                 if all([corner_p_l, corner_p_r, corner_r_l, corner_r_r]):
                      ...
                      QgsMessageLog.logMessage(f"Failed to calculate connector corners for {rwy_name} IHS base, skipping connector.", PLUGIN_TAG, level=Qgis.Warning)
                      connector = None
                    
                 if buffer_p and buffer_r and connector:
                      runway_strip_geom = QgsGeometry.unaryUnion([buffer_p, buffer_r, connector])
                 elif buffer_p and buffer_r: # Fallback if connector fails
                      QgsMessageLog.logMessage(f"Generating IHS base for {rwy_name} using only end buffers (connector failed).", PLUGIN_TAG, level=Qgis.Warning)
                      runway_strip_geom = QgsGeometry.unaryUnion([buffer_p, buffer_r])
                 else: # Buffers failed
                      QgsMessageLog.logMessage(f"Failed to generate buffers for {rwy_name} IHS base, skipping runway.", PLUGIN_TAG, level=Qgis.Warning)
                      continue # Skip this runway
          
                 if runway_strip_geom and not runway_strip_geom.isEmpty():
                      ...
                      if valid_geom and not valid_geom.isEmpty() and valid_geom.isGeosValid():
                           strip_outline_geoms.append(valid_geom)
                            ...
                            QgsMessageLog.logMessage(f"Generated strip outline for {rwy_name} is invalid, skipping.", PLUGIN_TAG, level=Qgis.Warning)
                 else:
                      QgsMessageLog.logMessage(f"Generated strip outline for {rwy_name} is empty, skipping.", PLUGIN_TAG, level=Qgis.Warning)
        ...
        # --- 3. Create IHS Layer ---
        ihs_layer_created = False # Flag to track success specifically
        try:
          # Check if initial geometry is valid before trying anything else
          if not ihs_base_geom or ihs_base_geom.isEmpty() or not ihs_base_geom.isGeosValid():
              QgsMessageLog.logMessage("Skipping IHS Layer creation: Initial base geometry invalid or empty.", PLUGIN_TAG, level=Qgis.Warning)
              ihs_base_geom = None # Invalidate if initial geom is bad
          else:
              # Geometry is valid, proceed with feature and layer creation
              ...
              # Optional: Add source height to reference
              # ref_text += f" (H={ols_dimensions.IHS_BASE_HEIGHT_AGL}m AGL)"
            
              fields = self._get_ols_fields("IHS")
              feature = QgsFeature(fields)
              feature.setGeometry(ihs_base_geom) # Use the valid geometry
            
              attr_map = {
                "RWY_Name": self.tr("Airport Wide"), "Surface": "IHS",
                "Elevation": IHS_ELEVATION_AMSL, "Height_AGL": ihs_base_height_agl,
                "Ref": ref_text, "Shape_Desc": "Convex Hull Approximation"
              }
              # Set attributes loop
              ...
              if layer:
                ...
                ihs_layer_created = True # Set flag
                QgsMessageLog.logMessage("IHS Layer Created (Convex Hull Approx).", PLUGIN_TAG, level=Qgis.Info)
                # <<< IMPORTANT: Do NOT reset ihs_base_geom here >>>
              else:
                # Layer creation helper returned None
                QgsMessageLog.logMessage("Failed to create IHS Layer (helper returned None).", PLUGIN_TAG, level=Qgis.Warning)
                # Do NOT invalidate ihs_base_geom here - the geom itself might still be fine
                # Let subsequent checks handle it if needed
                # ihs_base_geom = None # <<< REMOVE or COMMENT OUT this line >>>
                
        except Exception as e_ihs_layer:
          QgsMessageLog.logMessage(f"Error creating IHS Feature/Layer: {e_ihs_layer}", PLUGIN_TAG, level=Qgis.Critical)
          # Invalidate geometry only if an exception occurred during feature/layer creation
          ihs_base_geom = None # Invalidate on exception
        ...
        if ihs_base_geom: # Simpler check: if geometry exists and is valid (checked above)
                              ...
                              if temp_outer_geom.isGeosValid():
                                  ...
                                  try:
                                      ...
                                      if temp_conical_geom and not temp_conical_geom.isEmpty() and temp_conical_geom.isGeosValid():
                                          ...
                                          conical_total_height_agl = ihs_base_height_agl + height_extent_agl # Calculate total height AGL
                                          attr_map = {
                                              "RWY_Name": self.tr("Airport Wide"),
                                              "Surface": "Conical",
                                              "Elevation": conical_outer_elevation, # Top elevation AMSL
                                              "Height_AGL": conical_total_height_agl, # Total height AGL from Ref Datum
                                              "Slope_Perc": slope * 100.0,
                                              "Ref": ref,
                                              "Height_Extent": height_extent_agl # Height *above IHS*
                                            }
        ...
        QgsMessageLog.logMessage(f"DEBUG: State before Transitional check: ihs_base_geom is None? {ihs_base_geom is None}", PLUGIN_TAG, level=Qgis.Info) # Keep this debug log
        # Check prerequisites first
        if ihs_base_geom and IHS_ELEVATION_AMSL is not None:
          QgsMessageLog.logMessage(f"Transitional Surface generation check: IHS Geom available. (NOT IMPLEMENTED)", PLUGIN_TAG, level=Qgis.Info)
    ...
    def _generate_transitional_features(self,
      ...
      for runway_data in processed_runway_data_list:
        ...
        # Determine the index of the most restrictive type abbreviation
        type_order_abbr = ["", "NI", "NPA", "PA_I", "PA_II_III"] # List of abbreviations
        try:
              idx1 = type_order_abbr.index(type_abbr_1)
        except ValueError:
              idx1 = 1 # Default to NI index if not found
        try:
              idx2 = type_order_abbr.index(type_abbr_2)
        except ValueError:
              idx2 = 1 # Default to NI index if not found
        governing_type_index = max(idx1, idx2)

        type_order_full = ["", "Non-Instrument (NI)", "Non-Precision Approach (NPA)", "Precision Approach CAT I", "Precision Approach CAT II/III"]
        # Ensure type_order_full has the same order and length as type_order_abbr
        if governing_type_index < len(type_order_full):
              governing_type_str_full = type_order_full[governing_type_index]
              if not governing_type_str_full: # Handle empty string case for index 0
                  governing_type_str_full = "Non-Instrument (NI)"
        else:
              QgsMessageLog.logMessage(f"Warning: Could not determine governing full type string for index {governing_type_index}. Defaulting to NI.", PLUGIN_TAG, level=Qgis.Warning)
              governing_type_str_full = "Non-Instrument (NI)" # Fallback
          
        # <<< FIX: Pass the FULL descriptive string to get_ols_params >>>
        ...
        if not trans_params or 'slope' not in trans_params or trans_params['slope'] <= 1e-9:
          QgsMessageLog.logMessage(f"Skipping Transitional for {runway_name}: No valid slope found for classification ('{governing_type_str_full}').", PLUGIN_TAG, level=Qgis.Warning)
          continue # Skip to next runway if generating Transitional per-runway here
                    # Or handle differently if this is for airport-wide calculation
...
# ============================================================
    ...
    def _generate_tocs(self, runway_data: dict, rwy_params: dict, arc_num: int, end_type: str,
        ...
        """
        # <<< DEBUG LOG 1 >>>
        QgsMessageLog.logMessage(f"DEBUG _generate_tocs Start: end_desig={end_desig}, origin_elevation={origin_elevation} (type: {type(origin_elevation)}), clearway_len={clearway_len} (type: {type(clearway_len)})", PLUGIN_TAG, level=Qgis.Info)
        ...
        # <<< DEBUG LOG 2 >>>
          QgsMessageLog.logMessage(f"DEBUG _generate_tocs Params Raw: {params}", PLUGIN_TAG, level=Qgis.Info)
        ...
        try:
          ...
          # <<< DEBUG LOG 3 >>>
          QgsMessageLog.logMessage(f"DEBUG _generate_tocs Params Retrieved: offset={origin_offset}, iw={inner_width}, div={divergence}, len={overall_length}, fw={final_width}, slope={slope}", PLUGIN_TAG, level=Qgis.Info)
        ...
        # <<< DEBUG LOG 6 (Before Height Calc) >>>
        QgsMessageLog.logMessage(f"DEBUG _generate_tocs Pre-Height: overall_length={overall_length}, slope={slope}", PLUGIN_TAG, level=Qgis.Info)
        height_agl = overall_length * slope # Should be float * float
        # Check origin_elevation before adding
        # <<< DEBUG LOG 7 (Before Elev Calc) >>>
        QgsMessageLog.logMessage(f"DEBUG _generate_tocs Pre-Elev: origin_elevation={origin_elevation}, height_agl={height_agl}", PLUGIN_TAG, level=Qgis.Info)
    ...
    def process_guideline_f(self, runway_data: dict, layer_group: QgsLayerTreeGroup) -> bool:
      ...
      QgsMessageLog.logMessage(f"DEBUG: Calling _get_physical_runway_endpoints for {runway_name} with disp1={disp_thr_1}, disp2={disp_thr_2}", PLUGIN_TAG, level=Qgis.Info) # Log inputs
      physical_endpoints_result = self._get_physical_runway_endpoints(
      ...
      QgsMessageLog.logMessage(f"DEBUG: Physical endpoints calculated successfully for {runway_name}", PLUGIN_TAG, level=Qgis.Info) # Log success
      ...
      QgsMessageLog.logMessage(f"DEBUG: Starting Primary End processing for {primary_desig}", PLUGIN_TAG, level=Qgis.Info)
      ...
      try:
        ...
        QgsMessageLog.logMessage(f"DEBUG: Before Inner Approach P lookup (Type: {type1_str}, ARC: {arc_num})", PLUGIN_TAG, level=Qgis.Info)
        inner_app_params_p = ols_dimensions.get_ols_params(arc_num, type1_str, 'InnerApproach')
        QgsMessageLog.logMessage(f"DEBUG: After Inner Approach P lookup (Params found: {inner_app_params_p is not None})", PLUGIN_TAG, level=Qgis.Info)
        if inner_app_params_p:
            QgsMessageLog.logMessage(f"DEBUG: Generating Inner Approach P geometry...", PLUGIN_TAG, level=Qgis.Info)
            ...
            if all(v is not None for v in [slope_p, start_dist_p, inner_app_length, inner_app_width]):
                 ...
                 if start_point_p:
                      ...
                      if inner_app_geom_p and not inner_app_geom_p.isEmpty() and inner_app_geom_p.isGeosValid():
                           ...
                           QgsMessageLog.logMessage(f"DEBUG: Inner Approach P feature created.", PLUGIN_TAG, level=Qgis.Info)
                      else:
        ...
        QgsMessageLog.logMessage(f"DEBUG: Before Main Approach P generation", PLUGIN_TAG, level=Qgis.Info)
        ...
        QgsMessageLog.logMessage(f"DEBUG: After Main Approach P generation (Sections: {len(section_features_p)}, Contours: {len(contour_features_p)})", PLUGIN_TAG, level=Qgis.Info)
        ...
        QgsMessageLog.logMessage(f"DEBUG: Before TOCS P generation (Ref Pt type: {type(phys_p_end)}, CW: {clearway2_len}, Az: {rwy_params['azimuth_p_r']}, Elev type: {type(origin_elev_tocs_p)})", PLUGIN_TAG, level=Qgis.Info)
        ...
        QgsMessageLog.logMessage(f"DEBUG: After TOCS P generation (Feature generated: {feat_tocs1 is not None})", PLUGIN_TAG, level=Qgis.Info)
      ...
      QgsMessageLog.logMessage(f"DEBUG: Starting Reciprocal End processing for {reciprocal_desig}", PLUGIN_TAG, level=Qgis.Info)
      ...
      try:
        ...
        QgsMessageLog.logMessage(f"DEBUG: Before Inner Approach R lookup (Type: {type2_str}, ARC: {arc_num})", PLUGIN_TAG, level=Qgis.Info)
        inner_app_params_r = ols_dimensions.get_ols_params(arc_num, type2_str, 'InnerApproach')
        QgsMessageLog.logMessage(f"DEBUG: After Inner Approach R lookup (Params found: {inner_app_params_r is not None})", PLUGIN_TAG, level=Qgis.Info)
        if inner_app_params_r:
            QgsMessageLog.logMessage(f"DEBUG: Generating Inner Approach R geometry...", PLUGIN_TAG, level=Qgis.Info)
                 ...
                 if start_point_r:
                      ...
                      if inner_app_geom_r and not inner_app_geom_r.isEmpty() and inner_app_geom_r.isGeosValid():
                           ...
                           for name, value in attr_map.items():
                               ...
                               if idx != -1: feat.setAttribute(idx, value)
                           inner_approach_features.append(feat)
                           QgsMessageLog.logMessage(f"DEBUG: Inner Approach R feature created.", PLUGIN_TAG, level=Qgis.Info)
                      else:
        ...
        QgsMessageLog.logMessage(f"DEBUG: Before Main Approach R generation", PLUGIN_TAG, level=Qgis.Info)
        ...
        QgsMessageLog.logMessage(f"DEBUG: After Main Approach R generation (Sections: {len(section_features_r)}, Contours: {len(contour_features_r)})", PLUGIN_TAG, level=Qgis.Info)
        ...
        # Log the arguments being passed
        QgsMessageLog.logMessage(f"DEBUG: Before TOCS R generation (Ref Pt type: {type(phys_p_start)}, CW: {clearway1_len}, Az: {rwy_params['azimuth_r_p']}, Elev type: {type(origin_elev_tocs_r)})", PLUGIN_TAG, level=Qgis.Info)
        ...
        QgsMessageLog.logMessage(f"DEBUG: After TOCS R generation (Feature generated: {feat_tocs2 is not None})", PLUGIN_TAG, level=Qgis.Info)
      ...
      # --- Layer Creation ---
      QgsMessageLog.logMessage(f"DEBUG: Starting layer creation for {runway_name}", PLUGIN_TAG, level=Qgis.Info)
      ...
      if inner_approach_features:
        ...
        QgsMessageLog.logMessage(f"DEBUG: Inner Approach layer created: {layer is not None}", PLUGIN_TAG, level=Qgis.Info)
      ...
      if approach_poly_features:
        ...
        if layer: overall_success = True
        QgsMessageLog.logMessage(f"DEBUG: Approach Sections layer created: {layer is not None}", PLUGIN_TAG, level=Qgis.Info)
      ...
      if approach_contour_features:
        ...
        if layer: overall_success = True
        QgsMessageLog.logMessage(f"DEBUG: Approach Contours layer created: {layer is not None}", PLUGIN_TAG, level=Qgis.Info)
      ...
      if tocs_poly_features:
        ...
        if layer: overall_success = True
        QgsMessageLog.logMessage(f"DEBUG: TOCS layer created: {layer is not None}", PLUGIN_TAG, level=Qgis.Info)
