...
class SafeguardingBuilder:
      ...
      if runway_input_list:
        ...
        if processed_runway_data_list and any_runway_base_data_ok:
          ...
          if physical_geom_group and protection_area_group: # Check both groups created ok
                ...
                QgsMessageLog.logMessage("Committing physical geometry & protection area layers...", plugin_tag, level=Qgis.Info)
                commit_errors = False
                project_root = QgsProject.instance().layerTreeRoot() # Get root node
                    ...
                    if layer and layer.isValid() and layer.isEditable():
                        if physical_geom_features_added.get(element_type, False):
                            if layer.commitChanges():
                                QgsMessageLog.logMessage(f"Committed changes for '{layer.name()}'.", plugin_tag, level=Qgis.Info)
                                project.addMapLayer(layer, False) # Add layer to project first
                              
                                # Find the target group node using the stored name
                                target_group_name = layer.customProperty("target_group_node")
                                target_group_node = project_root.findGroup(target_group_name) if target_group_name else None
                              
                                if target_group_node:
                                    node = target_group_node.addLayer(layer)
                                    if node:
                                        ...
                                        QgsMessageLog.logMessage(f"Warning: Failed add node for layer '{layer.name()}' to group '{target_group_name}'.", plugin_tag, level=Qgis.Warning)
                                else:
                                     QgsMessageLog.logMessage(f"Warning: Target group '{target_group_name}' not found for layer '{layer.name()}'. Layer added to project root.", plugin_tag, level=Qgis.Warning)
                                     # Optionally add to main_group as fallback? main_group.addLayer(layer)
                            else:
                                commit_errors = True
                            ...
                            layer.rollBack() # No features added, just rollback edit state
                    # else: Layer was None, invalid, or not editable.
                          
                if commit_errors:
                    self.iface.messageBar().pushMessage(self.tr("Warning"), self.tr("Commit errors occurred on one or more layers. Check logs."), level=Qgis.Warning, duration=7)
    ...
    def _final_feedback(self, main_group: Optional[QgsLayerTreeGroup], root_node: QgsLayerTreeNode,
                            ...
                            physical_protection_ok: bool): # Renamed parameter
        """Provides final user feedback."""
        if main_group is None: return
        project = QgsProject.instance()
        # Use the combined flag here
        anything_created = arp_ok or met_ok or rwy_base_ok or physical_protection_ok or guide_c_ok or guide_g_ok or guide_rwy_ok
        if anything_created:
          ...
          if total_runways_in_input > 0:
            ...
            if physical_protection_ok: details.append("physical/protection areas") # Updated detail
