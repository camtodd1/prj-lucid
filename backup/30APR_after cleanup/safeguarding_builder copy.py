...
class SafeguardingBuilder:
    ...
    def _calculate_reference_elevation_datum(self, arp_elevation: Optional[float], runway_data_list: List[dict]) -> Optional[float]:
      ...
      for rwy_data in runway_data_list:
        ...
        if not valid_thr or not valid_rec:
          missing_parts = []
          if not valid_thr: missing_parts.append("THR1")
          if not valid_rec: missing_parts.append("THR2")
      ...
      if not threshold_elevations:
        QgsMessageLog.logMessage(
          ...
          plugin_tag, level=Qgis.Critical
        )
      ...
      if missing_elev_rwy_summaries:
        ...
        if self.iface:
          self.iface.messageBar().pushMessage(
            self.tr("Warning"),
            self.tr("Missing threshold elevations for some runways. "
      ...
      if abs(arp_elevation - avg_thr_elev) <= 3.0:
        ...
        QgsMessageLog.logMessage("RED based on ARP Elevation (within 3m of average THR elev).", plugin_tag, level=Qgis.Info)
      else:
    ...
    def run_safeguarding_processing(self):
      ...
      self.successfully_generated_layers = []
      self.reference_elevation_datum = None # Reset RED
      ...
      if runway_input_list:
        ...
        if processed_runway_data_list and any_runway_base_data_ok:
            ...
            if physical_geom_group:
                ...
                layer_definitions = { # Keep definitions
                  'Runway': {'name': self.tr('Runway Pavement'), 'fields': common_fields},
                  'PreThresholdRunway': {'name': self.tr('Pre-Threshold Runway'), 'fields': pre_threshold_fields},
                  'PreThresholdArea': {'name': self.tr('Pre-Threshold Area'), 'fields': pre_threshold_fields},
                  'DisplacedThresholdMarking': {'name': self.tr('Displaced Threshold Markings'), 'fields': marking_fields, 'geom_type': 'LineString'},
                  'PreThresholdAreaMarking': {'name': self.tr('Pre-Threshold Area Markings'), 'fields': marking_fields, 'geom_type': 'LineString'},
                  'Shoulder': {'name': self.tr('Runway Shoulders'), 'fields': common_fields},
                  'Stopway': {'name': self.tr('Stopways'), 'fields': stopway_resa_fields},
                  'GradedStrip': {'name': self.tr('Runway Graded Strips'), 'fields': common_fields},
                  'OverallStrip': {'name': self.tr('Runway Overall Strips'), 'fields': common_fields},
                  'RESA': {'name': self.tr('Runway End Safety Areas (RESA)'), 'fields': stopway_resa_fields}
                }
                style_key_map = { # Keep map
                  "Runway": "Runway Pavement", "PreThresholdRunway": "PreThreshold Runway", "PreThresholdArea": "PreThreshold Area",
                  ...
                  "OverallStrip": "Runway Overall Strips", "RESA": "Runway End Safety Areas (RESA)"
                }
                ...
                for element_type, definition in layer_definitions.items():
                    ...
                    if layer.isValid() and layer.dataProvider() and layer.dataProvider().addAttributes(definition['fields']):
                        ...
                        if layer.startEditing():
                            ...
                            # Warning: Failed to start editing, layer unusable.
                            QgsMessageLog.logMessage(f"Warning: Failed startEditing for physical layer '{layer_display_name}'. Layer will not be populated.", plugin_tag, level=Qgis.Warning)
                            physical_layers[element_type] = None # Mark as unusable
                    else:
                        # Critical: Layer setup failed.
                        prov_ok = layer.isValid() and layer.dataProvider()
                        attr_ok = prov_ok # Placeholder, assume attribute add failure if provider ok but overall failed
                        if prov_ok and not layer.dataProvider().addAttributes(definition['fields']): attr_ok = False
                        QgsMessageLog.logMessage(f"Critical: Failed initialize physical layer '{layer_display_name}' (Valid={layer.isValid()}, ProvOK={prov_ok}, AttrAddOK={attr_ok})", plugin_tag, level=Qgis.Critical)
                        physical_layers[element_type] = None # Mark as unusable
                ...
                for rwy_data in processed_runway_data_list:
                    ...
                    try:
                        # Generate physical geometry (logs inside helper)
                        generated_elements = self.generate_physical_geometry(rwy_data)
                        if generated_elements is None: # Helper function indicates failure
                             QgsMessageLog.logMessage(f"Skipping feature addition for {runway_name_log}: Geometry generation failed.", plugin_tag, level=Qgis.Warning)
                             continue
                  
                        # --- Start Refined Feature Addition Loop ---
                        for element_type, geometry, attributes in generated_elements:
                            target_layer = physical_layers.get(element_type)
                          
                            # Basic check: Is there a target layer and is it usable (editable)?
                            if target_layer is None or not target_layer.isEditable():
                                # Info/Debug: Layer was unusable from start or missing.
                                # QgsMessageLog.logMessage(f"Skipping add feature for {element_type} ({runway_name_log}): Target layer unusable or missing.", plugin_tag, level=Qgis.Info) # Optional Info log
                                continue
                          
                            # Check Geometry Validity
                            if geometry is None or geometry.isEmpty():
                                # Warning: Generator produced bad geometry.
                                QgsMessageLog.logMessage(f"Warning: Skipping add feature for {element_type} ({runway_name_log}): Generated geometry is null or empty.", plugin_tag, level=Qgis.Warning)
                                continue
                          
                            # Attempt to fix invalid geometry
                            if not geometry.isGeosValid():
                                QgsMessageLog.logMessage(f"Info: Fixing invalid geometry for {element_type} ({runway_name_log})...", plugin_tag, level=Qgis.Info)
                                geometry = geometry.makeValid()
                                # Check again after fixing
                                if geometry is None or geometry.isEmpty() or not geometry.isGeosValid():
                                    QgsMessageLog.logMessage(f"Warning: Skipping add feature for {element_type} ({runway_name_log}): Geometry invalid even after makeValid().", plugin_tag, level=Qgis.Warning)
                                    continue
                              
                            # Check for zero-length lines specifically
                            is_line = (element_type == 'DisplacedThresholdMarking' or element_type == 'PreThresholdAreaMarking')
                            if is_line:
                                try:
                                    geom_length = geometry.length()
                                    if geom_length is None or geom_length < 1e-9: # Use tolerance
                                        QgsMessageLog.logMessage(f"Warning: Skipping add feature for {element_type} ({runway_name_log}): Line geometry length is zero or near-zero.", plugin_tag, level=Qgis.Warning)
                                        continue # Skip zero-length lines
                                except Exception as e_len:
                                    # Warning: Unexpected error checking length. Decide whether to skip.
                                    QgsMessageLog.logMessage(f"Warning: Error checking line geometry length for {element_type} ({runway_name_log}): {e_len}. Attempting to add anyway.", plugin_tag, level=Qgis.Warning)
                            ...
                            # Setting ID to -1 is generally not needed for addFeature and can be removed.
                            # feature.setId(-1) # Removed
                              
                            # Add the feature
                            add_ok, _ = target_layer.dataProvider().addFeatures([feature]) # Use addFeatures for consistency
                          
                            if add_ok:
                                physical_geom_features_added[element_type] = True
                                any_physical_geom_ok = True
                                # No need to log success for every single feature addition.
                            else:
                                # Warning: Log failure to add this specific feature.
                                QgsMessageLog.logMessage(f"Warning: Failed to add physical feature {element_type} for {runway_name_log}. Error: {target_layer.dataProvider().lastError().message()}", plugin_tag, level=Qgis.Warning)
                        # --- End Refined Feature Addition Loop ---
                              
                    except Exception as e_phys:
                        # Critical: Error processing *this specific runway* in the physical geometry loop.
                        QgsMessageLog.logMessage(f"Critical Error during physical geometry feature population for {runway_name_log}: {e_phys}\n{traceback.format_exc()}", plugin_tag, level=Qgis.Critical)
                        # Continue to next runway if possible
                        continue
                  
                # Commit Layers and add to group
                # Info: Log start of commit phase.
                ...
                for element_type, layer in physical_layers.items():
                    if layer and layer.isValid() and layer.isEditable(): # Check layer exists, is valid, and was successfully put into edit mode
                        if physical_geom_features_added.get(element_type, False):
                            # Commit changes if features were added
                            if layer.commitChanges():
                                project.addMapLayer(layer, False)
                                node = physical_geom_group.addLayer(layer)
                                if node:
                                    node.setName(layer.name()) # Sync node name
                                    self._apply_style(layer, self.style_map) # Apply style after commit/add
                                    self.successfully_generated_layers.append(layer)
                                else:
                                    # Warning: Layer committed but failed to add to tree group.
                                    QgsMessageLog.logMessage(f"Warning: Failed to add committed layer '{layer.name()}' node to the layer tree.", plugin_tag, level=Qgis.Warning)
                                    # Optional: Remove layer from project if node add fails?
                                    # if project.mapLayer(layer.id()): project.removeMapLayer(layer.id())
                            else:
                                commit_errors = True
                                # Warning: Log commit failure.
                                QgsMessageLog.logMessage(f"Warning: Commit FAILED for '{layer.name()}': {layer.commitErrors()}", plugin_tag, level=Qgis.Warning)
                                layer.rollBack() # Rollback on failed commit
                                # Info: Log rollback action.
                                QgsMessageLog.logMessage(f"Rolled back changes for '{layer.name()}'.", plugin_tag, level=Qgis.Info)
                        else:
                            # No features were successfully added, just roll back to end editing state cleanly.
                            # This is normal operation, not an error, so Info level is fine, or remove log.
                            # QgsMessageLog.logMessage(f"Info: No features added for '{layer.name()}' ({element_type}). Rolling back edit session.", plugin_tag, level=Qgis.Info) # Optional log
                            layer.rollBack()
                    # else: Layer was None, invalid, or not editable - skip commit/rollback. Error was logged during layer init.
                          
                if commit_errors:
                    # User feedback: Warn about commit errors.
                    self.iface.messageBar().pushMessage(self.tr("Warning"), self.tr("Commit errors occurred on one or more physical geometry layers. Check logs."), level=Qgis.Warning, duration=7)
            else:
                # Warning: Failed to create the subgroup.
                QgsMessageLog.logMessage("Failed to create 'Physical Geometry' subgroup.", plugin_tag, level=Qgis.Warning)
        ...
        # Enable Save/Export Buttons (keep this part)
        ...
        enable_buttons = bool(self.successfully_generated_layers) # Enable if *any* layer was created
        ...
        self._final_feedback( main_group, root, icao_code, arp_layer_created, met_layers_created_ok,
                              ...
                              len(runway_input_list), any_physical_geom_ok )
    ...
    def _process_runways_part1(self, main_group: QgsLayerTreeGroup, project: QgsProject,
        ...
        QgsMessageLog.logMessage(f"Processing {len(runway_input_list)} runway(s)...", plugin_tag, level=Qgis.Info)
      
        for runway_data in runway_input_list:
          index = runway_data.get('original_index', '?')
          short_runway_name = f"RWY_{index}_ERR" # Default error name
          ...
          try:
            ...
            if centreline_layer:
              ...
              QgsMessageLog.logMessage(f"Failed create/add centreline layer for {short_runway_name}.", plugin_tag, level=Qgis.Warning)
            ...
            QgsMessageLog.logMessage(f"Critical error processing base data for runway {short_runway_name} (Index {index}): {e}\n{traceback.format_exc()}", plugin_tag, level=Qgis.Critical)
    ...
    def _apply_style(self, layer: QgsVectorLayer, style_map: Dict[str, str]):
        ...
        try:
          ...
          if qml_filename:
            ...
            QgsMessageLog.logMessage(f"Info: No specific or default style key found for layer '{layer_name}'. QGIS default will apply.", plugin_tag, level=Qgis.Info)
          
        except Exception as e:
          # Catch errors in the overall style finding/path logic
          QgsMessageLog.logMessage(f"Critical Error applying style logic to '{layer_name}': {e}\n{traceback.format_exc()}", plugin_tag, level=Qgis.Critical)
    ...
    def _try_get_arp_elevation_from_layer(self, arp_layer: QgsVectorLayer) -> Optional[float]:
      ...
      try:
        ...
        # Fallback: Check for an 'Elevation' attribute
        elev_field_idx = feature.fieldNameIndex('Elevation')
        if elev_field_idx != -1:
    ...
    def _create_and_add_layer(self, geometry_type_str: str, internal_name_base: str,
            ...
            Logs warnings or errors on failure. Does NOT log on success to reduce verbosity.
            """
            plugin_tag = PLUGIN_TAG # Local variable
      
            if not features:
                # Info: Log reason for not creating the layer.
            ...
            if not target_crs or not target_crs.isValid():
                # Critical: Cannot proceed without a valid CRS.
                QgsMessageLog.logMessage(f"Cannot create layer '{display_name}': Invalid Project CRS.", plugin_tag, level=Qgis.Critical)
                return None
            ...
            if geometry_type_str not in valid_geom_types:
                # Critical: Invalid geometry type requested.
            ...
            # Construct layer URI (handle potential Z/M dimensions if needed, though memory layer often doesn't require them explicitly here)
            layer_uri = f"{geometry_type_str}?crs={target_crs.authid()}&index=yes"
            layer = QgsVectorLayer(layer_uri, layer_name_internal, "memory")
      
            if not layer or not layer.isValid():
                # Warning: Failed to create the basic layer object.
            ...
            if not provider or not provider.addAttributes(fields):
                # Warning: Provider invalid or failed to add attributes.
                QgsMessageLog.logMessage(f"Failed setup provider/attributes for '{display_name}'. Provider exists: {provider is not None}", plugin_tag, level=Qgis.Warning)
                return None
            layer.updateFields()
            ...
            if layer.startEditing():
                # Attempt to add features
                # Note: Using addFeatures is often more efficient than adding one by one in a loop
                add_ok, out_feats = provider.addFeatures(features)
              
                if add_ok:
                    # Attempt to commit
                    if layer.commitChanges():
                        layer.updateExtents() # Important after adding features
                        project.addMapLayer(layer, False)
                        ...
                        if node:
                            ...
                            return layer # Success! Layer created and added.
                        else:
                            # Warning: Added to project but failed to add node to group.
                            QgsMessageLog.logMessage(f"Warning: Layer '{display_name}' added to project but failed add node to the layer tree group.", plugin_tag, level=Qgis.Warning)
                            # Optional: Remove layer if node addition fails? Depends on desired behaviour.
                            # if project.mapLayer(layer.id()): project.removeMapLayer(layer.id())
                            return None # Treat node add failure as overall failure for this helper's contract.
                    else:
                        # Warning: Commit failed.
                        QgsMessageLog.logMessage(f"Failed commit changes for '{display_name}'. Error: {layer.commitErrors()}", plugin_tag, level=Qgis.Warning)
                        layer.rollBack() # Rollback on commit failure
                        return None # Commit failed
                else:
                    # Warning: addFeatures failed.
                    # Note: provider.lastError() might contain more details, depending on the provider
                    last_error_msg = provider.lastError().message() if hasattr(provider, 'lastError') else "Provider error"
                    QgsMessageLog.logMessage(f"Failed addFeatures for '{display_name}'. Error: {last_error_msg}", plugin_tag, level=Qgis.Warning)
                    layer.rollBack() # Rollback if addFeatures failed within an edit session
                    return None # AddFeatures failed
            else:
                # Warning: Failed to start editing session.
    ...
    def _create_centered_oriented_square(self, center_point: QgsPointXY, side_length: float, description: str = "Square") -> Optional[QgsGeometry]:
        ...
        if not center_point or side_length <= 0:
          ...
          QgsMessageLog.logMessage(f"Skipping '{description}': Invalid center point or side length ({side_length}).", plugin_tag, level=Qgis.Warning)
          return None
        try:
          half_side = side_length / 2.0
          # Calculate corners (keep logic)
          min_x = center_point.x() - half_side; max_x = center_point.x() + half_side
          min_y = center_point.y() - half_side; max_y = center_point.y() + half_side
          sw = QgsPointXY(min_x, min_y); se = QgsPointXY(max_x, min_y)
          ne = QgsPointXY(max_x, max_y); nw = QgsPointXY(min_x, max_y)
          ...
          QgsMessageLog.logMessage(f"Warning: Error during geometry calculation for '{description}': {e}", plugin_tag, level=Qgis.Warning)
          return None
    ...
    def create_runway_centreline_layer(self, point1: QgsPointXY, point2: QgsPointXY, runway_name: str,
        ...
        try:
          ...
          if not isinstance(point1, QgsPointXY) or not isinstance(point2, QgsPointXY):
            QgsMessageLog.logMessage(f"Cannot create Centreline for {runway_name}: Invalid input point types.", plugin_tag, level=Qgis.Warning)
            return None
          if point1.compare(point2, epsilon=1e-6):
            QgsMessageLog.logMessage(f"Cannot create Centreline for {runway_name}: Start and end points are identical.", plugin_tag, level=Qgis.Warning)
            return None
          ...
          if line_geom.isNull() or line_geom.isEmpty():
            QgsMessageLog.logMessage(f"Cannot create Centreline for {runway_name}: Generated line geometry is null or empty.", plugin_tag, level=Qgis.Warning)
            return None
          ...
          layer = self._create_and_add_layer(
            ...
            f"{self.tr('Runway')} {runway_name} {self.tr('Centreline')}",
            fields,
            [feature],
            layer_group,
          ...
          QgsMessageLog.logMessage(f"Critical error creating Centreline feature/geometry for {runway_name}: {e}\n{traceback.format_exc()}", plugin_tag, level=Qgis.Critical)
          return None
    ...
    def _create_offset_rectangle(self, start_point: QgsPointXY, outward_azimuth_degrees: float, far_edge_offset: float, zone_length_backward: float, half_width: float, description: str = "Offset Rectangle") -> Optional[QgsGeometry]:
      ...
      if start_point is None or half_width <= 0:
        QgsMessageLog.logMessage(f"Skipping '{description}': Invalid start point or half width ({half_width}).", plugin_tag, level=Qgis.Warning)
        return None
      try:
        ...
        if not far_edge_center or not near_edge_center:
          QgsMessageLog.logMessage(f"Warning: Failed center point projection for '{description}'.", plugin_tag, level=Qgis.Warning)
          return None
        ...
        if not all(p is not None for p in corner_points):
          QgsMessageLog.logMessage(f"Warning: Failed corner point projection for '{description}'.", plugin_tag, level=Qgis.Warning)
          return None
    ...
    def _create_runway_aligned_rectangle(self, point1: QgsPointXY, point2: QgsPointXY, extension_m: float, half_width_m: float, description: str = "Aligned Rectangle") -> Optional[QgsGeometry]:
        ...
        if not point1 or not point2 or half_width_m <= 0 or point1.compare(point2, 1e-6):
          QgsMessageLog.logMessage(f"Skipping '{description}': Invalid points or half width ({half_width_m}).", plugin_tag, level=Qgis.Warning)
          return None
        try:
          params = self._get_runway_parameters(point1, point2)
          if not params: QgsMessageLog.logMessage(f"Warning: Failed getting runway parameters for '{description}'.", plugin_tag, level=Qgis.Warning); return None # Already logged by helper
          ...
          if not rect_start_center or not rect_end_center: QgsMessageLog.logMessage(f"Warning: Failed center point projection for '{description}'.", plugin_tag, level=Qgis.Warning); return None
          corner_start_l = rect_start_center.project(half_width_m, params['azimuth_perp_l']); corner_start_r = rect_start_center.project(half_width_m, params['azimuth_perp_r'])
          corner_end_l = rect_end_center.project(half_width_m, params['azimuth_perp_l']); corner_end_r = rect_end_center.project(half_width_m, params['azimuth_perp_r'])
          corner_points = [corner_start_l, corner_start_r, corner_end_r, corner_end_l]
          if not all(p is not None for p in corner_points): QgsMessageLog.logMessage(f"Warning: Failed corner point projection for '{description}'.", plugin_tag, level=Qgis.Warning); return None
          return self._create_polygon_from_corners(corner_points, description)
        except Exception as e:
          QgsMessageLog.logMessage(f"Warning: Error during geometry calculation for '{description}': {e}", plugin_tag, level=Qgis.Warning)
          return None
    ...
    def _create_trapezoid(self, start_point: QgsPointXY, outward_azimuth_degrees: float, length: float, inner_half_width: float, outer_half_width: float, description: str = "Trapezoid") -> Optional[QgsGeometry]:
        ...
        if not start_point or length <= 0 or inner_half_width < 0 or outer_half_width < 0:
          QgsMessageLog.logMessage(f"Skipping '{description}': Invalid start point/length/widths.", plugin_tag, level=Qgis.Warning)
          return None
        try:
          ...
          if not outer_center: QgsMessageLog.logMessage(f"Warning: Failed center point projection for '{description}'.", plugin_tag, level=Qgis.Warning); return None
    ...
    def _calculate_strip_dimensions(self, runway_data: dict) -> dict:
        ...
        results = {'overall_width': None, 'graded_width': None, 'extension_length': None,
              'mos_overall_width_ref': "N/A", 'mos_graded_width_ref': "N/A",
              'mos_extension_length_ref': "N/A"}
        log_name = runway_data.get('short_name', f"RWY_{runway_data.get('original_index','?')}")
      
        arc_num_str = runway_data.get('arc_num')
        runway_type = runway_data.get('type1') # Assuming type1 governs strip dimensions
        runway_width = runway_data.get('width')
      
        if not arc_num_str or not runway_type:
          QgsMessageLog.logMessage(f"Strip calc skipped for {log_name}: Missing ARC number or Type 1.", plugin_tag, level=Qgis.Info)
          return results # Return defaults if essential info missing
      
        try:
          arc_num = int(arc_num_str)
        except (ValueError, TypeError):
          QgsMessageLog.logMessage(f"Strip calc skipped for {log_name}: Invalid ARC number '{arc_num_str}'.", plugin_tag, level=Qgis.Warning)
        ...
        if arc_num in [1, 2] and is_non_instrument_or_npa:
          results.update({'overall_width': 140.0, 'mos_overall_width_ref': "MOS T6.17(4) Code 1/2 NI/NPA"})
        elif arc_num in [1, 2, 3, 4]: # Covers Code 3/4 or PA Cat I/II/III
          results.update({'overall_width': 280.0, 'mos_overall_width_ref': "MOS T6.17(4) Code 3/4 or PA"})
      
        # Graded Width
        if arc_num == 1:
          results.update({'graded_width': 60.0, 'mos_graded_width_ref': "MOS T6.17(1) Code 1"})
        elif arc_num == 2:
          results.update({'graded_width': 80.0, 'mos_graded_width_ref': "MOS T6.17(1) Code 2"})
        elif arc_num in [3, 4]:
          # Ensure runway_width is usable for comparison
          try:
            width_val = float(runway_width) if runway_width is not None else 45.0 # Assume >=45 if missing
          except (ValueError, TypeError):
            QgsMessageLog.logMessage(f"Warning: Invalid runway width '{runway_width}' for graded strip calc on {log_name}. Assuming >= 45m.", plugin_tag, level=Qgis.Warning)
            width_val = 45.0 # Default assumption
            
          if width_val < 45.0:
            results.update({'graded_width': 90.0, 'mos_graded_width_ref': "MOS T6.17(1) Code 3/4 (<45m)"})
          else: # >= 45m or width missing/invalid
            results.update({'graded_width': 150.0, 'mos_graded_width_ref': "MOS T6.17(1) Code 3/4 (>=45m)"})
          
        # Extension Length
        is_non_instrument_code_1_or_2 = (runway_type == "Non-Instrument (NI)" and arc_num in [1, 2])
        if is_non_instrument_code_1_or_2:
          results.update({'extension_length': 30.0, 'mos_extension_length_ref': "MOS 6.2.5.6(a) NI Code 1/2"})
        else: # Includes NPA, PA, or NI Code 3/4
          results.update({'extension_length': 60.0, 'mos_extension_length_ref': "MOS 6.2.5.6(b) Other"})
    ...
    def _get_runway_parameters(self, thr_point: QgsPointXY, rec_thr_point: QgsPointXY) -> Optional[dict]:
        ...
        if not isinstance(thr_point, QgsPointXY) or not isinstance(rec_thr_point, QgsPointXY):
          QgsMessageLog.logMessage("Invalid threshold point types for runway parameter calculation.", plugin_tag, level=Qgis.Warning)
          return None
        if thr_point.compare(rec_thr_point, 1e-6):
          QgsMessageLog.logMessage("Threshold points are identical for runway parameter calculation.", plugin_tag, level=Qgis.Warning)
          return None
        ...
        try:
          ...
          if length is None or math.isnan(length) or length < 0 or \
            azimuth_p_r is None or math.isnan(azimuth_p_r) or \
            azimuth_r_p is None or math.isnan(azimuth_r_p):
    ...
    def _create_polygon_from_corners(self, corners: List[QgsPointXY], description: str = "Polygon") -> Optional[QgsGeometry]:
      ...
      if not corners or len(corners) < 3 or None in corners or not all(isinstance(p, QgsPointXY) for p in corners):
        QgsMessageLog.logMessage(f"Cannot create polygon '{description}': Invalid input corner points.", plugin_tag, level=Qgis.Warning)
        return None
      ...
      try:
        ...
        if exterior_ring.isEmpty():
          QgsMessageLog.logMessage(f"Cannot create polygon '{description}': Generated exterior ring is empty.", plugin_tag, level=Qgis.Warning)
          return None
        ...
        if geom.isNull() or geom.isEmpty():
          QgsMessageLog.logMessage(f"Cannot create polygon '{description}': Initial geometry is null or empty after QgsPolygon creation.", plugin_tag, level=Qgis.Warning)
          return None
        ...
        if not geom.isGeosValid():
          ...
          if not geom_valid or geom_valid.isNull() or geom_valid.isEmpty() or not geom_valid.isGeosValid():
            QgsMessageLog.logMessage(f"Cannot create polygon '{description}': Geometry invalid even after makeValid().", plugin_tag, level=Qgis.Warning)
            return None
          
          # Handle potential MultiPolygon result from makeValid()
          valid_poly_types = [Qgis.WkbType.Polygon, Qgis.WkbType.PolygonZ, Qgis.WkbType.PolygonM, Qgis.WkbType.PolygonZM]
          valid_multipoly_types = [Qgis.WkbType.MultiPolygon, Qgis.WkbType.MultiPolygonZ, Qgis.WkbType.MultiPolygonM, Qgis.WkbType.MultiPolygonZM]
          ...
          if geom_valid.wkbType() in valid_multipoly_types:
              ...
              for poly_rings in polygons:
                ...
                if poly_rings and poly_rings[0]: # Check if exterior ring exists
                  try:
                    ...
                    temp_poly = QgsPolygon(poly_rings[0], poly_rings[1:]) # Pass exterior and list of interiors
                    ...
                    QgsMessageLog.logMessage(f"Warning: Error processing part of MultiPolygon for '{description}': {e_multi}", plugin_tag, level=Qgis.Warning)
            ...
            if largest_poly_geom:
              ...
              QgsMessageLog.logMessage(f"Cannot create polygon '{description}': Failed to extract valid part from MultiPolygon after makeValid().", plugin_tag, level=Qgis.Warning)
              return None
          elif geom_valid.wkbType() not in valid_poly_types:
            # makeValid returned something other than Polygon or MultiPolygon
            QgsMessageLog.logMessage(f"Cannot create polygon '{description}': makeValid() resulted in unexpected geometry type '{geom_valid.wkbType()}'", plugin_tag, level=Qgis.Warning)
            return None
          else:
        ...
        if geom is None or geom.isNull() or geom.isEmpty() or not geom.isGeosValid():
          QgsMessageLog.logMessage(f"Cannot create polygon '{description}': Final geometry check failed (Null/Empty/Invalid).", plugin_tag, level=Qgis.Warning)
          return None
        ...
        QgsMessageLog.logMessage(f"Critical error in _create_polygon_from_corners for '{description}': {e}\n{traceback.format_exc()}", plugin_tag, level=Qgis.Critical)
        return None
    ...
    def _create_rectangle_from_start(self, start_center_point: QgsPointXY, outward_azimuth: float,
      ...
      if not start_center_point or length <= 0 or half_width < 0:
        QgsMessageLog.logMessage(f"Skipping '{description}': Invalid start point/length/half width.", plugin_tag, level=Qgis.Warning)
        return None
      try:
        end_center_point = start_center_point.project(length, outward_azimuth)
        if not end_center_point: QgsMessageLog.logMessage(f"Warning: Failed end point projection for '{description}'", plugin_tag, level=Qgis.Warning); return None
        azimuth_perp_r = (outward_azimuth + 90.0) % 360.0; azimuth_perp_l = (outward_azimuth - 90.0 + 360.0) % 360.0
        start_l = start_center_point.project(half_width, azimuth_perp_l); start_r = start_center_point.project(half_width, azimuth_perp_r)
        end_l = end_center_point.project(half_width, azimuth_perp_l); end_r = end_center_point.project(half_width, azimuth_perp_r)
        corners = [start_l, start_r, end_r, end_l]
        if not all(p is not None for p in corners): QgsMessageLog.logMessage(f"Warning: Failed corner point projection for '{description}'", plugin_tag, level=Qgis.Warning); return None
    ...
    def generate_physical_geometry(self, runway_data: dict) -> Optional[List[Tuple[str, QgsGeometry, dict]]]:
        ...
        try:
          ...
          if all([thr_l, thr_r, rec_l, rec_r]):
            ...
            if landing_pavement_geom:
              ...
              attributes = {'RWY_Name': runway_name, 'Type': 'Runway Pavement', 'Width_m': runway_width, 'Length_m': round(landing_length, 3), 'MOS_Ref': pavement_ref}
              generated_elements.append(('Runway', landing_pavement_geom, attributes))
          ...
          try:
            ...
            if all([p_start_l, p_start_r, p_end_l, p_end_r]):
              ...
              if geom:
                physical_refs = ols_dimensions.get_physical_refs()
                pavement_ref = physical_refs.get('pavement', "MOS 139 6.2.3")
                attributes = {'RWY_Name': runway_name, 'Type': f'Pre-Threshold Pavement ({primary_desig})', 'Width_m': runway_width, 'Length_m': round(disp_thr_1, 3), 'MOS_Ref': pavement_ref}
          ...
          try:
            ...
            if all([r_start_l, r_start_r, r_end_l, r_end_r]):
              ...
              if geom:
                physical_refs = ols_dimensions.get_physical_refs()
                pavement_ref = physical_refs.get('pavement', "MOS 139 6.2.3")
                attributes = {'RWY_Name': runway_name, 'Type': f'Pre-Threshold Pavement ({reciprocal_desig})', 'Width_m': runway_width, 'Length_m': round(disp_thr_2, 3), 'MOS_Ref': pavement_ref}
                pre_threshold_features.append(('PreThresholdRunway', geom, attributes))
              # else: Failure logged by helper
      ...
      if runway_width is not None and runway_width > 0:
        ...
        if pre_area_len_1 > 1e-6:
          try:
            ...
            if geom:
              attributes = {'RWY_Name': runway_name, 'Type': f'Pre-Threshold Area ({primary_desig})', 'Width_m': runway_width, 'Length_m': round(pre_area_len_1, 3), 'MOS_Ref': "N/A"}
        ...
        if pre_area_len_2 > 1e-6:
          try:
            ...
            if geom:
              attributes = {'RWY_Name': runway_name, 'Type': f'Pre-Threshold Area ({reciprocal_desig})', 'Width_m': runway_width, 'Length_m': round(pre_area_len_2, 3), 'MOS_Ref': "N/A"}
              pre_threshold_area_features.append(('PreThresholdArea', geom, attributes))
            # else: Failure logged by helper
      ...
      if disp_thr_1 > 1e-6:
        try:
          ...
          if line_geom and not line_geom.isEmpty():
            ...
            attributes = {'RWY_Name': runway_name, 'Type': 'Displaced Threshold Marking', 'End_Desig': primary_desig, 'Length_m': round(line_len, 3) if line_len else None, 'MOS_Ref': marking_ref}
      ...
      if disp_thr_2 > 1e-6:
        try:
          ...
          if line_geom and not line_geom.isEmpty():
            line_len = line_geom.length()
            attributes = {'RWY_Name': runway_name, 'Type': 'Displaced Threshold Marking', 'End_Desig': reciprocal_desig, 'Length_m': round(line_len, 3) if line_len else None, 'MOS_Ref': marking_ref}
            displaced_marking_features.append(('DisplacedThresholdMarking', line_geom, attributes))
      ...
      if pre_area_len_1 > 1e-6:
        try:
          ...
          if line_geom and not line_geom.isEmpty():
            line_len = line_geom.length()
            attributes = {'RWY_Name': runway_name, 'Type': 'Pre-Threshold Area Marking', 'End_Desig': primary_desig, 'Length_m': round(line_len, 3) if line_len else None, 'MOS_Ref': marking_ref}
      ...
      if pre_area_len_2 > 1e-6:
        try:
          ...
          if line_geom and not line_geom.isEmpty():
            line_len = line_geom.length()
            attributes = {'RWY_Name': runway_name, 'Type': 'Pre-Threshold Area Marking', 'End_Desig': reciprocal_desig, 'Length_m': round(line_len, 3) if line_len else None, 'MOS_Ref': marking_ref}
            pre_area_marking_features.append(('PreThresholdAreaMarking', line_geom, attributes))
      ...
      if shoulder_width is not None and shoulder_width > 0 and runway_width is not None and runway_width > 0:
        try:
          ...
          shoulder_attrs = {'RWY_Name': runway_name, 'Type': 'Runway Shoulder', 'Width_m': shoulder_width, 'Length_m': round(physical_length, 3), 'MOS_Ref': shoulder_ref}
        ...
        if strip_dims and all(strip_dims.get(dim) is not None for dim in ['overall_width', 'graded_width', 'extension_length']):
          ...
          if strip_end_center_p and strip_end_center_r:
            ...
            if graded_strip_geom:
              ...
              graded_attrs = {'RWY_Name': runway_name, 'Type': 'Graded Strip', 'Width_m': graded_width, 'Length_m': round(strip_length, 3), 'MOS_Ref': graded_ref}
            ...
            if overall_strip_geom:
              ...
              overall_attrs = {'RWY_Name': runway_name, 'Type': 'Overall Strip', 'Width_m': overall_width, 'Length_m': round(strip_length, 3), 'MOS_Ref': overall_ref}
      ...
      try:
        ...
        if resa_dims and resa_dims.get('required') and strip_end_center_p and strip_end_center_r and runway_width is not None and runway_width > 0:
          ...
          resa_base_attrs = {'RWY_Name': runway_name, 'Type': 'RESA', 'Length_m': resa_length, 'Width_m': resa_width, 'MOS_Ref': resa_ref}
    ...
    def _generate_airport_wide_ols(self, processed_runway_data_list: List[dict],
        ...
        if ihs_base_height_agl is None:
          QgsMessageLog.logMessage("Cannot generate Airport-Wide OLS: Failed to retrieve IHS base height parameter.", plugin_tag, level=Qgis.Critical)
          return False
        ...
        QgsMessageLog.logMessage("Generating individual runway strip outlines for IHS base...", plugin_tag, level=Qgis.Info)
      
        if not processed_runway_data_list:
          QgsMessageLog.logMessage("Cannot generate IHS base: No processed runway data available.", plugin_tag, level=Qgis.Warning)
        ...
        for i, rwy_data in enumerate(processed_runway_data_list):
          ...
          try: # Broad try block for processing a single runway's outline
            ...
            runway_width = rwy_data.get('width')
          
            # Robustly get displacement values
            disp_val_1 = rwy_data.get('thr_displaced_1'); disp_val_2 = rwy_data.get('thr_displaced_2')
            ...
            if not all([thr_point, rec_thr_point, arc_num_str]):
              ...
              QgsMessageLog.logMessage(f"Skipping {rwy_name} strip outline - Missing essential data (Points/ARC Str).", plugin_tag, level=Qgis.Warning); continue
              
            # Check ARC Number
            try: arc_num = int(arc_num_str); highest_arc_num = max(highest_arc_num, arc_num)
            except (ValueError, TypeError): QgsMessageLog.logMessage(f"Skipping {rwy_name} strip outline - Invalid ARC number '{arc_num_str}'.", plugin_tag, level=Qgis.Warning); continue
            ...
            if max(current_type1_idx, current_type2_idx) > highest_idx_overall: highest_precision_type_str = current_max_type_str
          
            # Check Runway Parameters
            rwy_params = self._get_runway_parameters(thr_point, rec_thr_point)
            if not rwy_params: QgsMessageLog.logMessage(f"Skipping {rwy_name} strip outline - Failed runway params.", plugin_tag, level=Qgis.Warning); continue
            ...
            if physical_endpoints_result is None:
              QgsMessageLog.logMessage(f"Skipping {rwy_name} strip outline - Failed physical endpoints calc.", plugin_tag, level=Qgis.Warning); continue
            phys_p_start, phys_p_end, _ = physical_endpoints_result
          
            # Check Strip Dimensions
            strip_dims = rwy_data.get('calculated_strip_dims')
            if not strip_dims:
              QgsMessageLog.logMessage(f"Skipping {rwy_name} strip outline - 'calculated_strip_dims' was not found or invalid: {repr(strip_dims)}.", plugin_tag, level=Qgis.Warning); continue
            ...
            if not is_width_valid or not is_ext_valid:
              QgsMessageLog.logMessage(f"Skipping {rwy_name} strip outline - invalid content in strip_dims (W:{strip_width}, E:{strip_ext}).", plugin_tag, level=Qgis.Warning); continue
            ...
            if not strip_end_p or not strip_end_r:
              QgsMessageLog.logMessage(f"Skipping {rwy_name} strip outline - failed strip end point projection.", plugin_tag, level=Qgis.Warning); continue
            ...
            if ihs_end_radius is None or not isinstance(ihs_end_radius, (int, float)) or ihs_end_radius <= 0:
              if isinstance(strip_width, (int, float)) and strip_width > 0:
                ...
                QgsMessageLog.logMessage(f"Info: Using fallback radius {ihs_end_radius:.2f}m for {rwy_name} IHS outline (based on strip width).", plugin_tag, level=Qgis.Info)
              else:
                QgsMessageLog.logMessage(f"Skipping {rwy_name} strip outline - Cannot determine valid IHS radius (Params:{ihs_params}, Width:{strip_width}).", plugin_tag, level=Qgis.Warning); continue
            ...
            try: # Generate the geometry
              ...
              if all([corner_p_l, corner_p_r, corner_r_l, corner_r_r]):
                connector = self._create_polygon_from_corners([corner_p_l, corner_p_r, corner_r_r, corner_r_l], f"Strip Connector {rwy_name}")
              ...
              if len(components) >= 2:
                ...
                if runway_strip_geom: # Check if union produced something
                  ...
                  else: QgsMessageLog.logMessage(f"Warning: Generated strip outline for {rwy_name} is invalid after makeValid(), skipping.", plugin_tag, level=Qgis.Warning)
              elif not components:
                QgsMessageLog.logMessage(f"Warning: Skipping strip outline for {rwy_name}: Failed to generate valid buffer/connector components.", plugin_tag, level=Qgis.Warning)
              # else: Only one component was valid, cannot form outline.
                
            except Exception as e_strip_geom:
              QgsMessageLog.logMessage(f"Error generating strip outline geom for {rwy_name}: {e_strip_geom}", plugin_tag, level=Qgis.Warning)
              
          except Exception as loop_body_error:
            current_rwy_id = rwy_data.get('short_name', rwy_data.get('original_index', f'Unknown Index {i}'))
            QgsMessageLog.logMessage(f"CRITICAL: Unexpected error processing runway {current_rwy_id} (Loop Index {i}) in Airport OLS strip outline loop: {loop_body_error}\n{traceback.format_exc()}", plugin_tag, level=Qgis.Critical)
        ...
        if not strip_outline_geoms:
          QgsMessageLog.logMessage("IHS Generation Failed: No valid runway strip outlines were generated.", plugin_tag, level=Qgis.Critical)
          QgsMessageLog.logMessage(f"(Info: Highest runway precision type detected: {highest_precision_type_str}, ARC: {highest_arc_num})", plugin_tag, level=Qgis.Info)
          return False
      
        QgsMessageLog.logMessage(f"Creating IHS base polygon from Convex Hull of {len(strip_outline_geoms)} strip outline(s)...", plugin_tag, level=Qgis.Info)
        try:
          merged_geom = QgsGeometry.unaryUnion(strip_outline_geoms)
          if not merged_geom or merged_geom.isEmpty(): raise ValueError("unaryUnion of strip outlines failed.")
          ihs_base_geom = merged_geom.convexHull()
          if not ihs_base_geom or ihs_base_geom.isEmpty(): raise ValueError("convexHull calculation failed.")
          if not ihs_base_geom.isGeosValid():
            QgsMessageLog.logMessage("IHS base convex hull is invalid, attempting makeValid()...", plugin_tag, level=Qgis.Info)
            ihs_base_geom = ihs_base_geom.makeValid()
          if not ihs_base_geom or not ihs_base_geom.isGeosValid(): raise ValueError("IHS base geometry invalid after makeValid().")
        except Exception as e_hull:
          QgsMessageLog.logMessage(f"IHS Generation Failed: Error during Convex Hull creation: {e_hull}", plugin_tag, level=Qgis.Critical)
          return False
      
        # --- 3. Create IHS Layer ---
        ihs_layer_created = False
        if ihs_base_geom:
          try:
            ...
            attr_map = {"RWY_Name": self.tr("Airport Wide"), "Surface": "IHS", "Elevation": IHS_ELEVATION_AMSL, "Height_AGL": ihs_base_height_agl, "Ref": ref_text, "Shape_Desc": "Convex Hull Approximation"}
            ...
            if layer: overall_success = True; ihs_layer_created = True
          except Exception as e_ihs_layer:
            QgsMessageLog.logMessage(f"Critical error creating IHS Feature/Layer: {e_ihs_layer}\n{traceback.format_exc()}", plugin_tag, level=Qgis.Critical)
            ihs_base_geom = None
            
        # --- 4. Generate Conical Surface & Contours ---
        conical_layer_created = False; contour_layer_created = False
        if ihs_base_geom:
          conical_params = ols_dimensions.get_ols_params(highest_arc_num, highest_precision_type_str, 'CONICAL')
          if conical_params:
            height_extent_agl = conical_params.get('height_extent_agl'); slope = conical_params.get('slope'); ref = conical_params.get('ref', 'MOS 139 8.2.19')
            if slope is not None and slope > 0 and height_extent_agl is not None and height_extent_agl > 0:
              ...
              QgsMessageLog.logMessage(f"Generating Conical Surface: Slope={slope*100:.1f}%, H Ext={height_extent_agl:.1f}m, Horiz Ext={horizontal_extent:.1f}m, Top Elev AMSL={conical_outer_elevation:.2f}m", plugin_tag, level=Qgis.Info)
              try:
                ...
                if temp_outer_geom and not temp_outer_geom.isEmpty():
                  ...
                  if temp_outer_geom.isGeosValid():
                    ...
                    if temp_conical_geom and not temp_conical_geom.isEmpty() and temp_conical_geom.isGeosValid():
                      fields = self._get_ols_fields("Conical"); feature = QgsFeature(fields); feature.setGeometry(temp_conical_geom)
                      conical_total_height_agl = ihs_base_height_agl + height_extent_agl
                      attr_map = {"RWY_Name": self.tr("Airport Wide"), "Surface": "Conical", "Elevation": conical_outer_elevation, "Height_AGL": conical_total_height_agl, "Slope_Perc": slope * 100.0, "Ref": ref, "Height_Extent": height_extent_agl}
                      for name, value in attr_map.items(): idx = fields.indexFromName(name);
                      if idx != -1: feature.setAttribute(idx, value)
                      ...
                      if layer: overall_success = True; conical_layer_created = True
                    else: QgsMessageLog.logMessage("Failed generate valid Conical ring geometry (difference/makeValid).", plugin_tag, level=Qgis.Warning)
                  else: QgsMessageLog.logMessage("Failed generate valid outer Conical buffer after makeValid.", plugin_tag, level=Qgis.Warning)
                else: QgsMessageLog.logMessage("Failed generate outer Conical buffer (buffer returned None/empty).", plugin_tag, level=Qgis.Warning)
              except Exception as e_conical: QgsMessageLog.logMessage(f"Error during Conical Surface generation: {e_conical}", plugin_tag, level=Qgis.Warning)
            elif height_extent_agl is not None and height_extent_agl <= 0: QgsMessageLog.logMessage("Skipping Conical Surface: Height extent zero or negative.", plugin_tag, level=Qgis.Info)
            else: QgsMessageLog.logMessage("Skipping Conical Surface: Invalid parameters (Slope/Height Extent).", plugin_tag, level=Qgis.Warning)
          else: QgsMessageLog.logMessage(f"Skipping Conical Surface: No params found for Code {highest_arc_num}, Type {highest_precision_type_str}.", plugin_tag, level=Qgis.Warning)
          
          # --- 4b. Generate Conical CONTOURS ---
          if conical_layer_created and ihs_base_geom and outer_conical_geom and slope and height_extent_agl and height_extent_agl > 0 and conical_outer_elevation is not None:
            ...
            previous_contour_geom = QgsGeometry(ihs_base_geom); ref = conical_params.get('ref', 'MOS 139 8.2.19')
            if IHS_ELEVATION_AMSL % CONICAL_CONTOUR_INTERVAL == 0: first_contour_elev_amsl = IHS_ELEVATION_AMSL + CONICAL_CONTOUR_INTERVAL
            else: first_contour_elev_amsl = math.ceil(IHS_ELEVATION_AMSL / CONICAL_CONTOUR_INTERVAL) * CONICAL_CONTOUR_INTERVAL
            ...
            while current_target_contour_elev_amsl <= conical_outer_elevation + 1e-6:
              contour_h_above_ihs = min(current_target_contour_elev_amsl - IHS_ELEVATION_AMSL, height_extent_agl)
              if contour_h_above_ihs < 1e-6 : current_target_contour_elev_amsl += CONICAL_CONTOUR_INTERVAL; continue
              try:
                ...
                if outer_geom and not outer_geom.isEmpty():
                  ...
                  if outer_geom.isGeosValid():
                    if previous_contour_geom and not previous_contour_geom.isGeosValid(): previous_contour_geom = previous_contour_geom.makeValid()
                    if previous_contour_geom and previous_contour_geom.isGeosValid():
                      ...
                      if ring_geom and not ring_geom.isEmpty() and ring_geom.isGeosValid():
                        ...
                        feature = QgsFeature(fields); feature.setGeometry(ring_geom)
                  ...
                  else: QgsMessageLog.logMessage(f"Warning: makeValid failed for contour geom at elev={current_target_contour_elev_amsl}m.", plugin_tag, level=Qgis.Warning); break
                else: QgsMessageLog.logMessage(f"Warning: Buffer failed for contour geom at elev={current_target_contour_elev_amsl}m.", plugin_tag, level=Qgis.Warning); break
              ...
              if contour_h_above_ihs >= height_extent_agl - 1e-6: break
              current_target_contour_elev_amsl += CONICAL_CONTOUR_INTERVAL
            # Create contour layer if features exist
            if contour_features:
              fields = self._get_conical_contour_fields()
              contour_layer = self._create_and_add_layer("Polygon", f"OLS_Conical_Contours_{icao_code}", f"{self.tr('OLS')} Conical Contours {icao_code}", fields, contour_features, ols_layer_group, "OLS Conical Contour")
              if contour_layer: overall_success = True; contour_layer_created = True
              
        # --- 5. Generate Outer Horizontal Surface (OHS) ---
        ohs_layer_created = False
        ...
        if ohs_params:
          ...
          QgsMessageLog.logMessage(f"OHS required (Code {highest_arc_num}, Type {highest_precision_type_str}). Radius={radius}m, Height={height_agl}m AGL.", plugin_tag, level=Qgis.Info)
          if radius is not None and height_agl is not None and radius > 0:
            ohs_elevation_amsl = reference_elevation_datum + height_agl
            arp_point_xy = None; project = QgsProject.instance(); arp_layer_name = f"{icao_code} ARP"; arp_layers = project.mapLayersByName(arp_layer_name)
            if arp_layers: arp_feat = next(arp_layers[0].getFeatures(), None);
            if arp_feat and arp_feat.geometry() and not arp_feat.geometry().isNull(): arp_point_xy = arp_feat.geometry().asPoint()
            if arp_point_xy:
              try:
                center_geom = QgsGeometry.fromPointXY(arp_point_xy)
                ohs_full_circle_geom = center_geom.buffer(radius, 144) # Use high segments
                if ohs_full_circle_geom and not ohs_full_circle_geom.isEmpty():
                  ...
                  if ohs_full_circle_geom.isGeosValid():
                    ...
                    if outer_conical_geom and outer_conical_geom.isGeosValid():
                      ...
                      try:
                        ...
                        if difference_geom:
                          ...
                          if difference_geom.isGeosValid() and not difference_geom.isEmpty(): ohs_final_geom = difference_geom
                          else: QgsMessageLog.logMessage("Warning: Difference op for OHS resulted in invalid/empty geometry. Using full circle.", plugin_tag, level=Qgis.Warning)
                    ...
                    else: QgsMessageLog.logMessage("Info: Conical outer boundary not available or invalid for OHS difference. Using full OHS circle.", plugin_tag, level=Qgis.Info)
                    
                    fields = self._get_ols_fields("OHS"); feature = QgsFeature(fields); feature.setGeometry(ohs_final_geom)
                    ...
                    for name, value in attr_map.items(): idx = fields.indexFromName(name);
                    if idx != -1: feature.setAttribute(idx, value)
                    layer = self._create_and_add_layer("Polygon", f"OLS_OHS_{icao_code}", f"{self.tr('OLS')} OHS {icao_code}", fields, [feature], ols_layer_group, "OLS OHS")
                    if layer: overall_success = True; ohs_layer_created = True
            ...
            else: QgsMessageLog.logMessage("Skipping OHS generation: Could not find ARP point.", plugin_tag, level=Qgis.Warning)
          else: QgsMessageLog.logMessage("Skipping OHS generation: Invalid parameters (Radius/Height AGL).", plugin_tag, level=Qgis.Warning)
        ...
        transitional_layer_created = False
        if ihs_base_geom and IHS_ELEVATION_AMSL is not None:
          ...
          try:
            ...
            if target_crs and target_crs.isValid():
              transitional_features = self._generate_transitional_features(processed_runway_data_list, IHS_ELEVATION_AMSL, target_crs)
              if transitional_features:
                transitional_fields = self._get_ols_fields("Transitional")
                trans_layer = self._create_and_add_layer("Polygon", f"OLS_Transitional_{icao_code}", f"{self.tr('OLS')} Transitional {icao_code}", transitional_fields, transitional_features, ols_layer_group, "OLS Transitional")
                if trans_layer: overall_success = True; transitional_layer_created = True
            else: QgsMessageLog.logMessage("Skipping Transitional: Invalid Project CRS.", plugin_tag, level=Qgis.Warning)
          except Exception as e_trans:
            QgsMessageLog.logMessage(f"Error during Transitional Surface generation/layer addition: {e_trans}\n{traceback.format_exc()}", plugin_tag, level=Qgis.Critical)
        else: QgsMessageLog.logMessage("Skipping Transitional Surface generation: IHS geometry or elevation not available/valid.", plugin_tag, level=Qgis.Info)
      
        # --- Final Log ---
        QgsMessageLog.logMessage(f"Finished Airport-Wide OLS Generation.", plugin_tag, level=Qgis.Info)
    ...
    def _get_elevation_at_point_along_gradient(self,
        ...
        try:
            ...
            if line_geom.isNull():
                QgsMessageLog.logMessage("Failed elevation interpolation: Line geometry is null.", plugin_tag, level=Qgis.Warning)
                return None
            ...
            if line_primitive is None:
                 QgsMessageLog.logMessage("Failed elevation interpolation: Could not get geometry primitive.", plugin_tag, level=Qgis.Warning)
                 return None
            ...
            if result_tuple is None or len(result_tuple) < 2:
                 QgsMessageLog.logMessage("Failed elevation interpolation: closestSegment returned None or invalid tuple.", plugin_tag, level=Qgis.Warning)
                 return None
          
            closest_point_qgsp = result_tuple[1]
          
            if not isinstance(closest_point_qgsp, QgsPoint):
                QgsMessageLog.logMessage(f"Failed elevation interpolation: closestSegment did not return QgsPoint as second element (got {type(closest_point_qgsp)}).", plugin_tag, level=Qgis.Warning)
                return None
            ...
            return interpolated_elev
             ...
             QgsMessageLog.logMessage(f"AttributeError in elevation interpolation: {e_attr}. API mismatch?", plugin_tag, level=Qgis.Critical)
             return None
        except TypeError as e_type:
            QgsMessageLog.logMessage(f"Critical TypeError in elevation interpolation: {e_type}. Check code.", plugin_tag, level=Qgis.Critical)
            return None
        except Exception as e:
            QgsMessageLog.logMessage(f"Warning: Error in elevation interpolation: {e}", plugin_tag, level=Qgis.Warning)
            return None
    ...
    def _generate_transitional_features(self,
      ...
      QgsMessageLog.logMessage("Starting Transitional Surface feature generation...", plugin_tag, level=Qgis.Info)
      transitional_features: List[QgsFeature] = []
      transitional_fields = self._get_ols_fields("Transitional")

      if IHS_ELEVATION_AMSL is None: # Should be caught before calling, but double-check
          QgsMessageLog.logMessage("Skipping Transitional features: IHS Elevation is missing.", plugin_tag, level=Qgis.Warning)
          return []
      ...
      for runway_data in processed_runway_data_list:
          ...
          for end_idx, (end_desig, end_type, end_thr_pt, outward_az) in enumerate([
            ...
            current_section_start_pt = None; current_section_start_width = 0.0
            for i, section_params in enumerate(approach_sections_params):
              length = section_params.get('length', 0.0); divergence = section_params.get('divergence', 0.0)
              if length <= 0: continue
              if i == 0:
                start_dist = section_params.get('start_dist_from_thr', 0.0); start_width = section_params.get('start_width', 0.0)
                if start_width <= 0: break
                current_section_start_pt = end_thr_pt.project(start_dist, outward_az); current_section_start_width = start_width
              else:
                if current_section_start_pt is None: break
              if not current_section_start_pt: break
              start_hw = current_section_start_width / 2.0; end_width = current_section_start_width + (2 * length * divergence); end_hw = end_width / 2.0
              end_pt = current_section_start_pt.project(length, outward_az)
              if not end_pt: break
              az_perp_l = (outward_az + 270.0) % 360.0; az_perp_r = (outward_az + 90.0) % 360.0
              p_start_l = current_section_start_pt.project(start_hw, az_perp_l); p_start_r = current_section_start_pt.project(start_hw, az_perp_r)
              p_end_l = end_pt.project(end_hw, az_perp_l); p_end_r = end_pt.project(end_hw, az_perp_r)
              if all([p_start_l, p_end_l, p_start_r, p_end_r]):
                edge_l = QgsLineString([p_start_l, p_end_l]); edge_r = QgsLineString([p_start_r, p_end_r])
              ...
              current_section_start_pt = end_pt; current_section_start_width = end_width
      ...
      for runway_data in processed_runway_data_list:
          ...
          if not all([runway_name, thr_point, rec_thr_point, arc_num_str, calculated_strip_dims, thr_elev is not None, rec_thr_elev is not None]):
              QgsMessageLog.logMessage(f"Skipping Transitional features for {runway_name}: Missing required data.", plugin_tag, level=Qgis.Warning); continue
          try: arc_num = int(arc_num_str)
          except ValueError: QgsMessageLog.logMessage(f"Skipping Transitional features for {runway_name}: Invalid ARC.", plugin_tag, level=Qgis.Warning); continue
          rwy_params = self._get_runway_parameters(thr_point, rec_thr_point)
          if not rwy_params or rwy_params['length'] < 1e-6: QgsMessageLog.logMessage(f"Skipping Transitional features for {runway_name}: Invalid runway params.", plugin_tag, level=Qgis.Warning); continue
          primary_desig, reciprocal_desig = runway_name.split('/') if '/' in runway_name else ("THR1", "THR2")
        
          # --- Get Transitional Slope (keep logic as before) ---
          type_abbr_1 = ols_dimensions.get_runway_type_abbr(type1_str); type_abbr_2 = ols_dimensions.get_runway_type_abbr(type2_str)
          type_order_abbr = ["", "NI", "NPA", "PA_I", "PA_II_III"]; type_order_full = ["", "Non-Instrument (NI)", "Non-Precision Approach (NPA)", "Precision Approach CAT I", "Precision Approach CAT II/III"]
          try: idx1 = type_order_abbr.index(type_abbr_1)
          except ValueError: idx1 = 1
          try: idx2 = type_order_abbr.index(type_abbr_2)
          except ValueError: idx2 = 1
          governing_type_index = max(idx1, idx2)
          if governing_type_index < len(type_order_full): governing_type_str_full = type_order_full[governing_type_index] if type_order_full[governing_type_index] else "Non-Instrument (NI)"
          else: governing_type_str_full = "Non-Instrument (NI)"
          ...
          if not trans_params or 'slope' not in trans_params or trans_params['slope'] <= 1e-9:
              QgsMessageLog.logMessage(f"Skipping Transitional features for {runway_name}: No valid slope found for classification ('{governing_type_str_full}').", plugin_tag, level=Qgis.Warning); continue
          transitional_slope = trans_params['slope']; transitional_ref = trans_params.get('ref', "MOS 139 8.2.17 (Verify)")
        
          # --- Recalculate Overall Strip Geometry (keep logic) ---
          strip_overall_width = calculated_strip_dims.get('overall_width'); strip_extension = calculated_strip_dims.get('extension_length')
          if strip_overall_width is None or strip_extension is None: QgsMessageLog.logMessage(f"Skipping Transitional features for {runway_name}: Missing calc strip dims.", plugin_tag, level=Qgis.Warning); continue
          strip_overall_half_width = strip_overall_width / 2.0
          strip_end_p = thr_point.project(strip_extension, rwy_params['azimuth_r_p']); strip_end_r = rec_thr_point.project(strip_extension, rwy_params['azimuth_p_r'])
          if not strip_end_p or not strip_end_r: QgsMessageLog.logMessage(f"Skipping Transitional features for {runway_name}: Failed strip end points.", plugin_tag, level=Qgis.Warning); continue
          strip_corner_p_l = strip_end_p.project(strip_overall_half_width, rwy_params['azimuth_perp_l']); strip_corner_p_r = strip_end_p.project(strip_overall_half_width, rwy_params['azimuth_perp_r'])
          strip_corner_r_l = strip_end_r.project(strip_overall_half_width, rwy_params['azimuth_perp_l']); strip_corner_r_r = strip_end_r.project(strip_overall_half_width, rwy_params['azimuth_perp_r'])
          if not all([strip_corner_p_l, strip_corner_p_r, strip_corner_r_l, strip_corner_r_r]): QgsMessageLog.logMessage(f"Skipping Transitional features for {runway_name}: Failed strip corners.", plugin_tag, level=Qgis.Warning); continue
          strip_edge_l = QgsLineString([strip_corner_p_l, strip_corner_r_l]); strip_edge_r = QgsLineString([strip_corner_p_r, strip_corner_r_r])
        
          # --- Generate Strip Transitional Sides (keep logic, logs warnings on failure) ---
          for side_label, strip_edge, outward_azimuth in [('L', strip_edge_l, rwy_params['azimuth_perp_l']), ('R', strip_edge_r, rwy_params['azimuth_perp_r'])]:
                if not strip_edge or strip_edge.isEmpty(): continue
                p_start_qgsp = strip_edge.startPoint(); p_end_qgsp = strip_edge.endPoint()
                p_start_xy = QgsPointXY(p_start_qgsp.x(), p_start_qgsp.y()); p_end_xy = QgsPointXY(p_end_qgsp.x(), p_end_qgsp.y())
                z_start = self._get_elevation_at_point_along_gradient(p_start_xy, thr_point, rec_thr_point, thr_elev, rec_thr_elev, target_crs)
                z_end = self._get_elevation_at_point_along_gradient(p_end_xy, thr_point, rec_thr_point, thr_elev, rec_thr_elev, target_crs)
                if z_start is None or z_end is None: QgsMessageLog.logMessage(f"Failed calc Z for strip edge {side_label} on {runway_name}", plugin_tag, level=Qgis.Warning); continue
                if z_start >= IHS_ELEVATION_AMSL and z_end >= IHS_ELEVATION_AMSL: continue
                h_dist_start = max(0.0, (IHS_ELEVATION_AMSL - z_start) / transitional_slope); h_dist_end = max(0.0, (IHS_ELEVATION_AMSL - z_end) / transitional_slope)
                p_upper_start = p_start_xy.project(h_dist_start, outward_azimuth); p_upper_end = p_end_xy.project(h_dist_end, outward_azimuth)
                if not p_upper_start or not p_upper_end: QgsMessageLog.logMessage(f"Failed projecting upper points for strip edge {side_label} on {runway_name}", plugin_tag, level=Qgis.Warning); continue
                ...
                if poly_geom:
                  feat = QgsFeature(transitional_fields); feat.setGeometry(poly_geom)
                  attr_map = {"RWY_Name": runway_name, "Surface": "Transitional", "Section_Desc": "Strip Side", "Side": side_label, "Slope_Perc": transitional_slope * 100.0, "Ref": transitional_ref}
                  for name, value in attr_map.items(): idx = transitional_fields.indexFromName(name);
          ...
          for end_idx, (end_desig, end_type, end_thr_pt, end_thr_elev, outward_az) in enumerate([ (primary_desig, type1_str, thr_point, thr_elev, rwy_params['azimuth_r_p']), (reciprocal_desig, type2_str, rec_thr_point, rec_thr_elev, rwy_params['azimuth_p_r']) ]):
              approach_sections_params = ols_dimensions.get_ols_params(arc_num, end_type, 'APPROACH')
              if not approach_sections_params: continue
              current_section_start_elev = end_thr_elev; current_section_start_pt_ctr = None; prev_section_length = 0.0
              for i, section_params in enumerate(approach_sections_params):
                  section_length = section_params.get('length', 0.0); section_slope = section_params.get('slope', 0.0)
                  if section_length <= 0: continue
                  if i == 0: start_dist = section_params.get('start_dist_from_thr', 0.0); current_section_start_pt_ctr = end_thr_pt.project(start_dist, outward_az)
                  else:
                      if current_section_start_pt_ctr: current_section_start_pt_ctr = current_section_start_pt_ctr.project(prev_section_length, outward_az)
                      else: break
                  if not current_section_start_pt_ctr: break
                  ...
                  if section_end_elev is None: continue
                  for side_label, outward_perp_azimuth in [('L', (outward_az + 270.0) % 360.0), ('R', (outward_az + 90.0) % 360.0)]:
                      ...
                      pa_start = approach_edge.startPoint(); pa_end = approach_edge.endPoint()
                      za_start = current_section_start_elev; za_end = section_end_elev # Approx
                      p1_3d = QgsPoint(pa_start.x(), pa_start.y(), za_start); p2_3d = QgsPoint(pa_end.x(), pa_end.y(), za_end)
                      ...
                      if clipped_segment:
                          ...
                          h_dist_app_start = max(0.0, (IHS_ELEVATION_AMSL - za_start_clipped) / transitional_slope); h_dist_app_end = max(0.0, (IHS_ELEVATION_AMSL - za_end_clipped) / transitional_slope)
                          ...
                          if pa_upper_start and pa_upper_end:
                              corners = [QgsPointXY(pa_start_clipped.x(), pa_start_clipped.y()), QgsPointXY(pa_end_clipped.x(), pa_end_clipped.y()), pa_upper_end, pa_upper_start]
                              ...
                              if poly_geom:
                                  ...
                                  attr_map = {"RWY_Name": runway_name, "Surface": "Transitional", "End_Desig": end_desig, "Section_Desc": f"Approach Sec {i+1}", "Side": side_label, "Slope_Perc": transitional_slope * 100.0, "Ref": transitional_ref}
                                  for name, value in attr_map.items(): idx = transitional_fields.indexFromName(name);
                                  if idx != -1: feat.setAttribute(idx, value)
                                  transitional_features.append(feat)
                              # else: Failure logged by helper
                  current_section_start_elev = section_end_elev; prev_section_length = section_length
                
      # Info: Log completion of feature generation.
      QgsMessageLog.logMessage(f"Finished Transitional Surface feature generation. Created {len(transitional_features)} features.", plugin_tag, level=Qgis.Info)
...
# ============================================================
    ...
    def _generate_tocs(self, runway_data: dict, rwy_params: dict, arc_num: int, end_type: str,
          ...
          origin_elevation: Optional[float]) -> Optional[QgsFeature]: # Elevation at the OTHER threshold
        """
        ...
        try:
          ...
          essential_params = [origin_offset, inner_width, divergence, overall_length, final_width, slope]
          if any(p is None for p in essential_params):
            missing_keys = [k for k, v in params.items() if v is None and k in ['origin_offset', 'inner_edge_width', 'divergence', 'length', 'final_width', 'slope']]
            QgsMessageLog.logMessage(f"Essential TOCS parameters missing or None ({', '.join(missing_keys)}) for Code {arc_num}, End {end_desig}.", PLUGIN_TAG, level=Qgis.Critical)
            return None
          ...
          QgsMessageLog.logMessage(f"Error processing/converting TOCS parameters for Code {arc_num}, End {end_desig}: {e_param}", PLUGIN_TAG, level=Qgis.Critical)
          return None
        ...
        if clearway_len > 1e-6: # Use tolerance
            ...
            if not projected_clearway_end:
                 QgsMessageLog.logMessage(f"Failed calc TOCS clearway end point {end_desig}", PLUGIN_TAG, level=Qgis.Warning)
                 return None
        ...
        if not start_point:
          QgsMessageLog.logMessage(f"Failed calc TOCS start point after offset {end_desig}", PLUGIN_TAG, level=Qgis.Warning)
          return None
        ...
        total_offset_from_phys_end = runway_phys_end_point.distance(start_point) if start_point and runway_phys_end_point else None
    ...
    def process_guideline_f(self, runway_data: dict, layer_group: QgsLayerTreeGroup) -> bool:
      ...
      if not all([thr_point, rec_thr_point, layer_group, arc_num_str]):
        QgsMessageLog.logMessage(f"OLS App/TOCS skipped {runway_name}: Missing essential data (THR points, group, ARC).", PLUGIN_TAG, level=Qgis.Warning)
        return False
      try:
          arc_num = int(arc_num_str)
      except (ValueError, TypeError):
        QgsMessageLog.logMessage(f"OLS App/TOCS skipped {runway_name}: Invalid ARC number '{arc_num_str}'.", PLUGIN_TAG, level=Qgis.Warning)
        return False
      ...
      try:
          ...
          QgsMessageLog.logMessage(f"Invalid Clearway 1 value '{cw1_val}' for {runway_name}, using 0.0.", PLUGIN_TAG, level=Qgis.Warning)
          clearway1_len = 0.0
      try:
          clearway2_len = float(cw2_val) if cw2_val is not None else 0.0
      except (ValueError, TypeError):
          QgsMessageLog.logMessage(f"Invalid Clearway 2 value '{cw2_val}' for {runway_name}, using 0.0.", PLUGIN_TAG, level=Qgis.Warning)
          clearway2_len = 0.0
      ...
      try:
          ...
          QgsMessageLog.logMessage(f"Invalid Displacement 1 value '{disp_val_1}' for {runway_name}, using 0.0.", PLUGIN_TAG, level=Qgis.Warning)
          disp_thr_1 = 0.0
      ...
      try:
          ...
          QgsMessageLog.logMessage(f"Invalid Displacement 2 value '{disp_val_2}' for {runway_name}, using 0.0.", PLUGIN_TAG, level=Qgis.Warning)
          disp_thr_2 = 0.0
      # --- End CORRECTED displacement threshold handling ---
        
      # --- Calculate Physical Endpoints ---
      physical_endpoints_result = self._get_physical_runway_endpoints(
          thr_point, rec_thr_point, disp_thr_1, disp_thr_2, rwy_params # Pass validated floats
      )
      if physical_endpoints_result is None:
          QgsMessageLog.logMessage(f"Skipping OLS App/TOCS for {runway_name}: Failed to calculate physical endpoints.", PLUGIN_TAG, level=Qgis.Warning)
          return False
      phys_p_start, phys_p_end, _ = physical_endpoints_result # Get the physical start and end points
      ...
      try:
                 ...
                 if start_point_p:
                      ...
                      if inner_app_geom_p and not inner_app_geom_p.isEmpty() and inner_app_geom_p.isGeosValid():
                           ...
                           attr_map = {
                               ...
                               "OuterW_m": inner_app_width, "Origin_Offset": start_dist_p
                           }
                           for name, value in attr_map.items():
                               idx = fields.indexFromName(name)
                               if idx != -1: feat.setAttribute(idx, value)
                           ...
                           QgsMessageLog.logMessage(f"DEBUG: Failed to generate valid Inner Approach P geometry.", PLUGIN_TAG, level=Qgis.Info)
                 else:
                      QgsMessageLog.logMessage(f"DEBUG: Failed to calculate Inner Approach P start point.", PLUGIN_TAG, level=Qgis.Info)
            else:
                 QgsMessageLog.logMessage(f"DEBUG: Skipping Inner Approach P generation due to missing parameters.", PLUGIN_TAG, level=Qgis.Info)
        ...
        section_features_p, contour_features_p = self._generate_approach_surface(
            ...
            thr_point, outward_azimuth_p, # Uses landing threshold thr_point
        ...
        if feat_tocs1: tocs_poly_features.append(feat_tocs1)
         ...
         QgsMessageLog.logMessage(f"CRITICAL Error caught within Primary End ({primary_desig}) processing block: {e}", PLUGIN_TAG, level=Qgis.Critical)
      ...
      try:
                 ...
                 if start_point_r:
                      ...
                      if inner_app_geom_r and not inner_app_geom_r.isEmpty() and inner_app_geom_r.isGeosValid():
                           ...
                           attr_map = {
                               ...
                               "Length_m": inner_app_length, "InnerW_m": inner_app_width,
                               "OuterW_m": inner_app_width, "Origin_Offset": start_dist_r
                           }
                           for name, value in attr_map.items():
                               idx = fields.indexFromName(name)
                               if idx != -1: feat.setAttribute(idx, value)
                           ...
                           QgsMessageLog.logMessage(f"DEBUG: Failed to generate valid Inner Approach R geometry.", PLUGIN_TAG, level=Qgis.Info)
                 else:
                      QgsMessageLog.logMessage(f"DEBUG: Failed to calculate Inner Approach R start point.", PLUGIN_TAG, level=Qgis.Info)
            else:
                 QgsMessageLog.logMessage(f"DEBUG: Skipping Inner Approach R generation due to missing parameters.", PLUGIN_TAG, level=Qgis.Info)
        ...
        section_features_r, contour_features_r = self._generate_approach_surface(
            ...
            rec_thr_point, outward_azimuth_r, # Uses landing threshold rec_thr_point
        ...
        if feat_tocs2: tocs_poly_features.append(feat_tocs2)
         ...
         QgsMessageLog.logMessage(f"CRITICAL Error caught within Reciprocal End ({reciprocal_desig}) processing block: {e}", PLUGIN_TAG, level=Qgis.Critical)
         QgsMessageLog.logMessage(f"Traceback:\n{traceback.format_exc()}", PLUGIN_TAG, level=Qgis.Critical)
         # Decide whether to stop all processing or just skip this runway end
         # return False # Option: Stop everything
      ...
      QgsMessageLog.logMessage(f"Finished Runway OLS (InnerApp/App/TOCS) processing for {runway_name}.", PLUGIN_TAG, level=Qgis.Success)
      return overall_success
...
# ============================================================
    ...
    def process_raoa(self, runway_data: dict, layer_group: QgsLayerTreeGroup) -> bool:
        ...
        if not all([thr_point, rec_thr_point, layer_group]): return False # Basic check
        rwy_params = self._get_runway_parameters(thr_point, rec_thr_point)
        if rwy_params is None: return False # Helper logs error
      
        RAOA_LENGTH = 300.0; RAOA_WIDTH = 120.0; RAOA_HALF_WIDTH = RAOA_WIDTH / 2.0
        ...
        if features_to_add:
          ...
          layer_created = self._create_and_add_layer("Polygon", internal_name_base, layer_name_display, fields, features_to_add, layer_group, style_key)
    ...
    def process_taxiway_separation(self, runway_data: dict, layer_group: QgsLayerTreeGroup) -> bool:
        ...
        runway_name = runway_data.get('short_name', f"RWY_{runway_data.get('original_index','?')}")
        thr_point = runway_data.get('thr_point'); rec_thr_point = runway_data.get('rec_thr_point')
        arc_num_str = runway_data.get('arc_num'); arc_let_raw = runway_data.get('arc_let')
        type1_str = runway_data.get('type1', ''); type2_str = runway_data.get('type2', '')
        ...
        if not arc_let_raw or not isinstance(arc_let_raw, str) or not arc_let_raw.strip():
          QgsMessageLog.logMessage(f"Taxiway Sep skipped {runway_name}: ARC Letter not provided or invalid ('{arc_let_raw}').", plugin_tag, level=Qgis.Warning); return False
        ...
        runway_length = rwy_params['length']; line_length = runway_length * 1.5; extension = (line_length - runway_length) / 2.0
        line_start_cl = thr_point.project(extension, rwy_params['azimuth_r_p']); line_end_cl = line_start_cl.project(line_length, rwy_params['azimuth_p_r'])
        if not line_start_cl or not line_end_cl: QgsMessageLog.logMessage(f"Failed calc taxiway sep line start/end points for {runway_name}", plugin_tag, level=Qgis.Warning); return False
        ...
        # Create Layer
        if features_to_add:
          layer_name_display = f"Taxiway Separation {runway_name}"; internal_name_base = f"TaxiwaySep_{runway_name.replace('/', '_')}"
          fields = self._get_taxiway_separation_fields(); style_key = "Taxiway Separation Line"
          layer_created = self._create_and_add_layer("LineString", internal_name_base, layer_name_display, fields, features_to_add, layer_group, style_key)
          return layer_created is not None
        else:
