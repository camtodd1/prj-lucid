...
class SafeguardingBuilder:
    ...
    def _generate_airport_wide_ols(self, processed_runway_data_list: List[dict],
        ...
        # --- END OF SECTION 5 (OHS) ---

        # --- 6. Generate Transitional Surfaces (Placeholder) ---
        # This section should be placed AFTER OHS generation
        # QgsMessageLog.logMessage(f"DEBUG: State before Transitional check: ihs_base_geom is None? {ihs_base_geom is None}, isEmpty? {ihs_base_geom.isEmpty() if ihs_base_geom else 'N/A'}", PLUGIN_TAG, level=Qgis.Info)
        if ihs_base_geom and not ihs_base_geom.isEmpty():
              QgsMessageLog.logMessage(f"Transitional Surface generation NOT IMPLEMENTED. (Using IHS Convex Hull Approx)", PLUGIN_TAG, level=Qgis.Info)
        else:
              QgsMessageLog.logMessage("Skipping Transitional Surface generation: IHS geometry not available/valid.", PLUGIN_TAG, level=Qgis.Info)
