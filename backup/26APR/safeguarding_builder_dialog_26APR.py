...
class SafeguardingBuilderDialog(QtWidgets.QDialog, FORM_CLASS):
    ...
    def get_all_input_data(self) -> Optional[Dict[str, Any]]:
        ...
        final_data.update({
            ...
            'arp_easting': arp_e,
            'arp_northing': arp_n,
            'arp_elevation': arp_elev, # <<< MODIFICATION: Add elevation to dict >>>
            'met_point': met_pt
        })
        ...
        if not self._runway_groups:
            ...
            for index, group_widget in sorted(self._runway_groups.items()):
                ...
                if validated_runway:
                    # (Already handled inside _validate_runway_data, just checking keys)
                    if 'thr_elev_1' not in validated_runway: validated_runway['thr_elev_1'] = None # Ensure keys exist
    ...
    def _get_global_inputs(self) -> Tuple[Optional[str], Optional[QgsPointXY], Optional[float], Optional[float], Optional[float], Optional[QgsPointXY]]:
        ...
        try:
            ...
            # If coords are valid or not provided, check elevation
            if arp_elev_lineEdit: # <<< MODIFICATION: Check elevation widget exists >>>
                    arp_elev_str = arp_elev_lineEdit.text().strip()
                    if arp_elev_str: # Only validate if text is entered
                        try:
                                ...
                                return None, None, None, None, None, None # <<< MODIFICATION: Update return tuple >>>
                    else:
                        # Empty elevation is acceptable, pass None
                        arp_elev = None
                        QgsMessageLog.logMessage("ARP Elevation field is empty.", DIALOG_LOG_TAG, level=Qgis.Info)
            else: # Elevation widget not found
                    arp_elev = None
                    QgsMessageLog.logMessage("Warning: ARP Elevation widget not found.", DIALOG_LOG_TAG, level=Qgis.Warning)
    ...
    def load_input_data(self):
        ...
        try:
            ...
            required = ["icao_code", "arp_easting", "arp_northing", "arp_elevation", "runways"]
