...
class RunwayWidgetGroup(QtWidgets.QGroupBox):
    ...
    def _setup_ui(self):
        ...
        self.thr_displaced_1_le.setValidator(
        ...
        self.thr_displaced_2_le.setValidator(
            self.distance_validator
        )  # Use distance validator
        ...
        self.thr_pre_area_1_le.setToolTip(
            "Length of pre-threshold area (cleared/graded) for primary end (meters). Leave blank if none."
        )
        self.thr_pre_area_1_le.setValidator(
            self.distance_validator
        )  # Use distance validator
        ...
        self.thr_pre_area_2_le.setToolTip(
            "Length of pre-threshold area (cleared/graded) for reciprocal end (meters). Leave blank if none."
        )
        self.thr_pre_area_2_le.setValidator(
            self.distance_validator
        )  # Use distance validator
        ...
        label_rwy_azim_text = QtWidgets.QLabel("Azimuth (°):")
        ...
        detailsLayout.addWidget(label_arc_num, current_details_row, 0)
        detailsLayout.addWidget(self.arc_num_combo, current_details_row, 1)
        current_details_row += 1
        ...
        detailsLayout.addWidget(label_arc_let, current_details_row, 0)
        detailsLayout.addWidget(self.arc_let_combo, current_details_row, 1)
        current_details_row += 1
        runway_types = [
            "",
            "Non-Instrument (NI)",
        ...
        detailsLayout.addWidget(self.type1_lbl, current_details_row, 0)
        detailsLayout.addWidget(self.type1_combo, current_details_row, 1)
        current_details_row += 1
        ...
        detailsLayout.addWidget(self.type2_lbl, current_details_row, 0)
        detailsLayout.addWidget(self.type2_combo, current_details_row, 1)
        current_details_row += 1
        ...
        self.remove_button.setToolTip(f"Remove this runway")
    ...
    def set_input_data(self, data: Dict[str, str]):
        ...
        try:
            ...
            if self.suffix_combo:
                idx = self.suffix_combo.findText(
            ...
            if self.type1_combo:
                ...
                self.type1_combo.setCurrentIndex(idx if idx >= 0 else 0)
            if self.type2_combo:
                idx = self.type2_combo.findText(
                    data.get("type2", ""), QtCore.Qt.MatchFlag.MatchFixedString
                )
                self.type2_combo.setCurrentIndex(idx if idx >= 0 else 0)
        finally:
            for w in widgets_to_block:
                if w:
    ...
    def update_display_labels(self, results: Dict[str, str]):
        ...
        if self.type1_lbl:
            self.type1_lbl.setText(
                results.get("type1_label_text", "(Primary End) Type:")
            )
        if self.type2_lbl:
    ...
    def clear_inputs(self):
        ...
        try:
            ...
            if self.thr_east_le:
                self.thr_east_le.clear()
            if self.thr_north_le:
                self.thr_north_le.clear()
            if self.rec_east_le:
                self.rec_east_le.clear()
            if self.rec_north_le:
                self.rec_north_le.clear()
            if self.thr_elev_1_le:
                self.thr_elev_1_le.clear()
            if self.thr_elev_2_le:
                self.thr_elev_2_le.clear()
            if self.thr_displaced_1_le:
                self.thr_displaced_1_le.clear()
            if self.thr_displaced_2_le:
            ...
            if self.width_le:
                self.width_le.clear()
            if self.shoulder_le:
                self.shoulder_le.clear()
            if self.arc_num_combo:
                self.arc_num_combo.setCurrentIndex(0)
            if self.arc_let_combo:
                self.arc_let_combo.setCurrentIndex(0)
            if self.type1_combo:
...
class SafeguardingBuilderDialog(QtWidgets.QDialog, FORM_CLASS):
    ...
    def add_cns_row(self):
        ...
        if not cns_table:
            QgsMessageLog.logMessage(
                "Add CNS Row Error: Table widget 'table_cns_facility' not found.",
                DIALOG_LOG_TAG,
                level=Qgis.Critical,
            )
            return
    ...
    def remove_cns_rows(self):
        ...
        if not cns_table:
            QgsMessageLog.logMessage(
                "Remove CNS Row Error: Table widget 'table_cns_facility' not found.",
                DIALOG_LOG_TAG,
                level=Qgis.Critical,
            )
            return
    ...
    def _get_cns_manual_data(self) -> Optional[List[Dict[str, Any]]]:
        ...
        if not cns_table:
            QgsMessageLog.logMessage(
                "CNS Validation Error: Table widget not found.",
                DIALOG_LOG_TAG,
                level=Qgis.Critical,
            )
        ...
        for row in range(total_rows):
            ...
            try:
                ...
                if valid_row:
                    ...
                    x_str, y_str = (x_item.text().strip() if x_item else ""), (
                        y_item.text().strip() if y_item else ""
                    )
                    if not x_str or not y_str:
                        ...
                        try:
                            ...
                            error_in_row = True
                            QgsMessageLog.logMessage(
                                f"CNS Row {row+1}: Invalid coords.",
                                DIALOG_LOG_TAG,
                                level=Qgis.Warning,
                            )
                ...
                if valid_row:
                    ...
                    if elev_str:
                        try:
                            ...
                            facility_elevation = None
                            error_in_row = True
                            QgsMessageLog.logMessage(
                                f"CNS Row {row+1}: Invalid elev, ignoring.",
                                DIALOG_LOG_TAG,
                                level=Qgis.Warning,
                            )
            except Exception as e:
            ...
            if valid_row and point_geom_project_crs:
                facility_id = f'Manual_{row+1}_{facility_type.replace(" ", "_").replace("-","").replace("(","").replace(")","")}'[
                    :50
                ]
        ...
        if rows_with_errors > 0:
            QMessageBox.warning(
                self,
                "CNS Data Warning",
                f"{rows_with_errors} CNS row(s) had errors/invalid data and were skipped or data ignored.\n({skipped_rows} total skipped/incomplete). Check Log.",
            )
        ...
        if cns_table:
            for row in range(cns_table.rowCount()):
                try:
                    ...
                    x_item, y_item, elev_item = (
                        cns_table.item(row, 1),
                        cns_table.item(row, 2),
                        cns_table.item(row, 3),
                    )
                    x_txt, y_txt, elev_txt = (
                        (x_item.text() if x_item else ""),
                        (y_item.text() if y_item else ""),
                        (elev_item.text() if elev_item else ""),
                    )
                    cns_list.append(
                        {
                            "type": type_txt,
    ...
    def load_input_data(self):
        ...
        try:
            ...
            required = ["icao_code", "runways"]
            missing = [k for k in required if k not in loaded_data]
            if missing:
            ...
            if arp_east_le:
                arp_east_le.setText(loaded_data.get("arp_easting", ""))
            if arp_north_le:
                arp_north_le.setText(loaded_data.get("arp_northing", ""))
            if arp_elev_le:
                arp_elev_le.setText(loaded_data.get("arp_elevation", ""))
            if met_east_le:
            ...
            if loaded_runways_list:
                first_runway_index = (
                    min(self._runway_groups.keys()) if self._runway_groups else None
                )
                first_group = (
                    self._runway_groups.get(first_runway_index)
                    if first_runway_index is not None
                    else None
                )
                if first_group:
                ...
                for runway_data_item in loaded_runways_list[1:]:
                    try:
                        ...
                        if new_group:
                            ...
                            QgsMessageLog.logMessage(
                                f"Load Error: Group {new_index} missing after add.",
                                DIALOG_LOG_TAG,
                                level=Qgis.Warning,
                            )
            ...
            if cns_table and loaded_cns_list:
                ...
                for item_data in loaded_cns_list:
                    ...
                    try:
                        ...
                        cns_table.setItem(
                            row, 1, QTableWidgetItem(item_data.get("easting_x", ""))
                        )
                        cns_table.setItem(
                            row, 2, QTableWidgetItem(item_data.get("northing_y", ""))
                        )
                        cns_table.setItem(
                            row, 3, QTableWidgetItem(item_data.get("elevation", ""))
                        )
                    except Exception as e:
                        QgsMessageLog.logMessage(
